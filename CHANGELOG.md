...
- **compound + 4:4:4 sub-8x8 verified end-to-end** — `encode_pframe_lossless_layout` gains an optional second reference: leaves pairing `[ LAST, ALTREF ]` (the §6.3.18 fixed/variable layout under asymmetric-ALTREF sign biases) predict compound — the sub-8x8 per-`blockIdx` walk included, so the §8.5.2 `Round2( p0 + p1, 1 )` average runs per 4x4 cell — and the frame codes `reference_mode = SELECT`. Byte-exact end-to-end pins: a compound sub-8x8 P-frame (two-list per-cell NEWMV, compound NEARESTMV via the per-list §6.5.14 seeds, a full compound 8x8 leaf) drawing LAST from the keyframe and ALTREF from a hidden intra-only slot; a **4:4:4** sub-8x8 P-frame (full-resolution chroma reads each 4x4 cell's own `blockIdx` vector — no sub-8x8 MV averaging, a distinct §8.5.2 arm from 4:2:0); and a switchable per-block `interp_filter` on a sub-8x8 block (token order between the absent block-level `inter_mode` and the per-cell walk) (round 415)
