...
PARTITION_NONE / PARTITION_SPLIT decision via bytestream-level
inspection of the first SB's partition bit pattern: the uniform
fixture decodes to PARTITION_NONE (the tile's first symbol against
`PARTITION_PROBS[0][0]` reads `0`) and the per-quadrant fixture
decodes to PARTITION_SPLIT (reads `1, 1, 1`). The PSNR row for the
SPLIT fixture is intentionally low because divergent per-quadrant
motion combined with the encoder's smooth-content-hostile sub-pel
ME lands on spurious local minima; partition correctness is the test
signal, not absolute quality.
