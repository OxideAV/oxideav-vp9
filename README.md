...
## Encode support (round 49 + r-multiref + r-next-hp)
...
`encode_pframe_yuv_multi_ref(&EncoderParams, &YuvFrame, &ReferenceSet)`
(r-multiref) extends the P-frame path with a second optional
GOLDEN_FRAME reference; the encoder runs per-CU ME against both
references and picks the lower-SAD one, emitting the §6.4.5 single-ref
tree bits accordingly.
...
### P-frame inter encode (round 49 + r-next sub-pel ME + r-next partitions + r-next-8x8 16×16 HORZ/VERT + r-next-sub8 8×8 four-way RDO + r-multiref LAST/GOLDEN + r-next-hp 1/8-pel + r-next-near NEARESTMV/NEARMV)

`encode_pframe_yuv` emits a single-reference (LAST_FRAME) P-frame;
`encode_pframe_yuv_multi_ref` extends to LAST + optional GOLDEN per-CU
RDO. Both share the rest of the encode pipeline:

* **Quadtree partitions** per §6.4.16 / §6.5. Each 64×64 superblock
  may emit `PARTITION_NONE` or `PARTITION_SPLIT` into four 32×32
  ...
  shape. The 16×16 RDO additionally evaluates HORZ (2 × 16×8 ME)
  and VERT (2 × 8×16 ME) with a 1× rate penalty (1 extra inter
  block vs NONE) and SPLIT with 3× penalty (3 extra inter blocks).
* Three-stage motion estimation against the reconstructed reference
  ...
  3. **Quarter-pel** 8-neighbour refinement around the half-pel
     winner. (r-next)
  4. **1/8-pel (high-precision)** 8-neighbour refinement around the
     quarter-pel winner — only runs when
     `EncoderParams::allow_high_precision_mv = true`. (r-next-hp)
  Each refinement pass is iteration-bounded (≤ 2 passes) so per-block
  cost stays bounded. 1/4-pel is the standard VP9 baseline precision;
  the 1/8-pel stage + `hp` bit are an opt-in per-frame quality knob —
  flip `EncoderParams::allow_high_precision_mv = true` to engage it
  (the §6.2 `allow_high_precision_mv` header bit follows automatically).
* **Four inter modes** (r-next-near): `ZEROMV` / `NEARESTMV` /
  `NEARMV` / `NEWMV`. The picker runs after ME and selects the
  cheapest mode for the picked MV against the §6.5.12
  `find_best_ref_mvs` result the decoder will reproduce:
    * MV equals `RefListMv[0]` (non-zero) → `NEARESTMV` (2 bits,
      no MV delta).
    * MV equals `RefListMv[1]` (non-zero, count ≥ 2, ≠ `RefListMv[0]`)
      → `NEARMV` (3 bits, no MV delta).
    * Otherwise → `NEWMV` (3 bits + §6.4.19 MV joint + per-component
      class/bits/fr — `hp` bit conditional on
      `EncoderParams::allow_high_precision_mv`).
    * MV ≈ 0 (per `ME_NEWMV_GATE_SAD`) → `ZEROMV` (1 bit).
  The picker only emits NEAREST/NEAR for non-zero candidates so a
  zero `RefListMv[0]` never collides with ZEROMV's 1-bit win. Cell
  `y_mode` is updated to the picked mode so subsequent `mode_ctx`
  neighbour lookups on the encoder side mirror what the decoder will
  derive from the same wire. Saves typically 5–25 bits per block
  whenever neighbouring CUs share the ME-picked MV.
* **Per-CU reference-frame RDO** (r-multiref). When `ReferenceSet::golden`
  is present, every CU runs ME against both LAST and GOLDEN, then
  picks the lower-SAD reference (with a small bit-rate penalty
  charged against GOLDEN to account for its extra `single_ref_p2` bit).
  Per-block `ref_frame[0]` is emitted via the §6.4.5 single-ref tree:
  LAST writes 1 bit (`single_ref_p1 = 0`), GOLDEN writes 2 bits
  (`single_ref_p1 = 1` then `single_ref_p2 = 0`). DPB layout:
  ...
  slot 0). The §9.3.2 `single_ref_p1_ctx` / `single_ref_p2_ctx`
  neighbour contexts mirror the decoder VERBATIM so the bool stream
  stays in lock-step. ALTREF deferred — only LAST / GOLDEN are picked.
...
* `tx_mode = ONLY_4X4`, `interpolation_filter = 0` (EightTap, fixed —
  ...
  §6.3.12 compound-mode bit; r-multiref is still single-ref per CU —
  one ref per block, just chosen between LAST and GOLDEN).
...
| `r_next_partition` · per-quadrant shift (must pick SPLIT)        |  400 B  |   31 B  | **21.23 dB** |
| `r_next_partition` · 4-px translation (regression — picks NONE)  |  410 B  |   22 B  | **49.96 dB** |
| `r_next_8x8` · 16×16 textured patch (SPLIT → 8×8 NONE)            |  192 B  |  166 B  | **19.97 dB** |
| `r_next_8x8` · horizontal 16×8 stripe (must pick HORZ)            |  461 B  |   72 B  | shape-only   |
| `r_next_8x8` · vertical 8×16 stripe (must pick VERT)              |  462 B  |   74 B  | shape-only   |
| `r_next_8x8` · per-8×8 motion (≥ 0.5 dB gain over 16×16 baseline) |  400 B  |  176 B  | **18.49 dB** (+3.55 dB vs baseline 14.94 dB) |
| `r_multiref` · 4-frame A→B→A→A GOP (≥ 0.5 dB gain over LAST-only) | 636 B (key) | 346/186 B (P2/P3 multi) | **21.63 dB avg** (+1.51 dB vs LAST-only 20.11 dB) |
| `r_next_hp_mv` · 1/8-pel shift (EightTap phase-2)                  |  412 B  |   21 B  | **∞ dB (HP=ON) vs 52.56 dB (HP=OFF) → exact reconstruction once odd-1/8-pel MVs are reachable** |
| `r_next_near_mv` · 256×256 uniform 4-px shift (NEARESTMV picker) | 562 B | **262 B** (was 279 B pre-r78) | **12.74 dB** (q=64 deblock-aggressive; gate at 10 dB — picker engagement gated by byte-size delta, **-17 B / -6%**) |
...
PARTITION_NONE / PARTITION_SPLIT / HORZ / VERT decision via
bytestream-level inspection of the first SB's partition bit pattern
at the relevant level. The r-next-8x8 HORZ / VERT rows descend
through SB(0,0) → TL 32×32 → TL-of-TL 16×16 and assert the picked
shape; the r-next-8x8 regression row compares the actual encoder
PSNR against an `EncoderParams::debug_force_16x16_only` baseline (same
loop-filter / header / outer-partition path with the 16×16 picker
short-circuited to NONE) — a +0.5 dB minimum is the gate. The
r-multiref row runs a 4-frame GOP where frame 2 is orthogonal content
to the keyframe (so its skip=1 reconstruction is a warped stripe
excerpt, structurally wrong for frames 3+4 which return to the
keyframe content); the multi-ref encoder picks GOLDEN(=keyframe) for
nearly every CU in frames 3+4 and reconstructs them via ZEROMV-copy,
beating the LAST-only baseline by **+1.51 dB** average over frames 3+4.
...
* Sub-pel ME — round 49 + r-next + r-next-hp now cover half-, quarter-,
  and 1/8-pel ME refinement (the 1/8-pel stage is opt-in via
  `EncoderParams::allow_high_precision_mv`). No further sub-pel work
  pending on the ME side.
