...
## [0.0.14](https://github.com/OxideAV/oxideav-vp9/compare/v0.0.13...v0.0.14) - 2026-09-11

### Other

- encode_two_pass_structured fuzz target - eleventh harness: oracle-carrying round-trip over the structured two-pass entries (every Vp9GopConfig axis incl. scene-cut keyframe placement, adaptive group length and the coefficient election, at 4:2:0 / 4:4:4 / 4:2:2 / 4:4:0 / 10-bit 4:2:0) across fuzz-derived geometry / budget / VBV / content with an optional mid-sequence scene cut: one report entry per packet, the first a keyframe, every display frame coded exactly once, every decoded packet within its budget (or at the q == 255 floor), show_existing_frame packets at most two bytes, the stream decoding to exactly the source frame count at the declared format; 1345 runs / 300 s clean locally, encode_gop_structures 2603 runs / 150 s and encode_adaptive_chain 818 runs / 150 s clean on the round-458 defaults; two tracked seeds; fuzz workflow roster refreshed; README test count + fuzz roster
- refine_leaf_mv_subpel - drop the unneeded mut on the memoising closure (clippy unused_mut, -D warnings)
- encoder profiling + byte-identical speedups - Criterion bench benches/encode.rs (structured GOP + plain chain at 96x64 x 6 frames, lossy keyframe at 256x160) and a sampling profile of the chain encoder (predict_inter under the sub-pel refinement 22 %, the integer full search, the forward DCT's per-coefficient cos( ) call, the writer); four speedups, each output-preserving: (1) the forward DCT's cosines come from a per-size table holding the exact doubles the inline expression produced (summation order unchanged); (2) the integer full search's SAD row loop bails out once the partial sum reaches the running best - a winner must be strictly below it, so no elected vector changes; (3) the §8.5.2.4 eight-tap horizontal pass reads its taps straight from the clipped reference row's slice whenever the window lies inside [0, lastX] (every interior column; the edge fallback keeps the per-tap Clip3), tap products accumulated in the same order - pinned by an equivalence test against the generic form across every kernel, scaled / unscaled steps and blocks straddling every plane edge; (4) the sub-pel descent memoises each candidate vector's SAD; MEASURED (release): structured GOP 220.5 -> 155.6 ms (27.2 -> 38.6 fps, -29 %), chain 191.7 -> 138.5 ms (31.3 -> 43.3 fps, -28 %), keyframe 256x160 49.1 -> 29.5 ms (20.4 -> 33.9 fps, -40 %); every corpus byte count and every staged-fixture identity pin unchanged; criterion joins dev-dependencies
- coefficient election over the §6.4.24 token tree - rdoq module + Vp9GopConfig::coefficient_rdo (default on; every adaptive-framing entry - chain, rc, two-pass, structured, resized, registry): each trial quantisation re-elects its levels rate-distortion-wise against the frame's OWN loaded §6.1.2 bank - per-cell token-tree costs (internal nodes above TWO through the §9.3.2 pareto( ) tail exactly as build_token_probs derives them), extra magnitude bits under CAT_PROBS, sign, and the more_coefs flags, with the §9.3.2 context replayed over the block's own already-elected tokens - against pixel-domain distortion measured through the crate's own §8.7 inverse transform (per transform size and coefficient position a unit-coefficient gain is taken from inverse_transform_2d once); two deterministic greedy passes: tail trimming (a trailing ONE and the zero run before it go when their bits outweigh the distortion they save) and level reduction in scan order (ONE -> ZERO also suppresses the next position's more_coefs flag per the §6.4.24 check_eob latch); lambda = qstep_px² / 16 per bit (measured against 1/8 and 1/32); integer-only arithmetic, the decoder untouched (elected levels are ordinary tokens through the existing writer and reconstruction mirror); MEASURED on the six-shape corpus against a five-quantizer plain scalar-quantiser RD curve: every sequence lands above the curve, +0.19..+1.87 dB at equal bytes, mean +4.8 % fewer bytes at equal PSNR; the two-pass chain at equal target: static 38.14 -> 38.81 dB, moving 41.70 -> 42.06, scene cut 36.09 -> 36.69; pinned by the RD-curve test, unit tests (measured gains, bank-following costs, a marginal trailing ONE trimmed while exact levels stay), and the corpus dump byte-exact through the black-box reference decoder; the entropy-model identity pin holds the election off (it reads the bank); fixed-quantiser fixture builders (ChainModel::Chained, entropy_adaptation = false) are byte-unchanged; content-dependent rate-control premises (static GOP motion exactly zero, keyframe strictly costliest) relaxed to bounded assertions
- scene-cut keyframe placement + adaptive alt-ref group length from first-pass statistics - the structured two-pass entries plan their second pass: Vp9GopConfig::scene_cut_keyframes (default on) places a keyframe on every display frame whose first-pass inter cost spikes (above twice the median inter cost, or at least two thirds of the first keyframe's intra cost) AND is confirmed by a probe keyframe encode at the first-pass quantizer to be at least three quarters of its intra cost - inter prediction bought nothing worth its syntax, so the keyframe is as cheap and resets the prediction chain (every slot refreshed, roles restarted, a new alt-ref segment); Vp9GopConfig::adaptive_group_length (default on) halves an alt-ref group whose frames' mean first-pass motion activity exceeds 48 eighth-pel until it fits or is a lone P-frame (a distant alt-ref predicts fast motion poorly; not applied under intra_only_altref, whose period is a refresh choice; hidden intra-only alt-refs are never cut candidates - their first-pass size is an intra cost); the structured encoder now codes an explicit GopPlan packet sequence (encode_packets over PlannedPacket { Key | HiddenAltref | Shown | ShowExisting }, encode_with = the structure's default plan, byte-identical - the staged round-452 fixtures still pin), mid-stream keyframes through the same commit path as the first; the second-pass allocation weights the planned packets with their first-pass cost (a placed keyframe: its probe intra cost); Vp9TwoPassFrame gains keyframe + frame; MEASURED at equal bytes vs planning off: seg-full scene-cut GOP 34.37 -> 35.03 dB (keyframe placed on the cut frame), pyramid 37.23 -> 37.52, tile-rows 36.77 -> 37.81, 4:4:4 32.60 -> 33.01, 4:2:2 34.18 -> 34.24, 4:4:0 34.53 -> 35.10, 10-bit 4:2:0 42.09 -> 42.35, 12-bit 4:2:2 41.55 -> 42.00, intra-only and static GOPs byte-identical (planning is a no-op there); pinned by the A/B in the shapes test, a static-GOP no-op test, and the env-gated dump with all ten streams byte-exact through the black-box reference decoder
- two-pass rate control over the structured GOP + the format matrix - encode_vp9_lossy_sequence_rc_two_pass_with(frames, w, h, cfg, target_bytes_per_frame, vbv_bytes) and _444/_422/_440/_hbd/_hbd_422/_hbd_440: every Vp9GopConfig axis (alt-ref pyramid, segmentation, tiles, intra-only alt-refs, entropy model, switchable filter) at every §7.2 format under the two-pass allocation; the StructuredGopEncoder is refactored into a probe / commit split (prepare_keyframe / prepare_pframe / prepare_intra_only_arf code + finish a frame at a quantizer against the read-only encoder state and return a PendingFrame carrying every persistent side effect - slot refresh, §7.2.6 field, §6.4.14 map, §7.2.8 baseline, §7.2.10 table, §6.1.2 bank products - that commit applies; EntropyModel gains the mutation-free begin_frame_view / bank_after_begin twins) and a per-packet quantizer policy (encode_with: FrameQ::Fixed or FrameQ::Budget bisected through bisect_q exactly as the chain), the fixed-quantizer path byte-identical (the five staged round-452 fixtures still pin their bytes); pass one records per-packet size + motion activity (StructuredGopEncoder::stats - the packet sequence depends only on frame count and structure, so first-pass statistics index the second pass one for one), pass two allocates the pool less the show_existing_frame packets proportionally to first-pass size (hidden alt-refs draw their share) under the VBV model with refills at DISPLAY points (a shown frame or the show_existing_frame packet presenting the alt-ref) and a structure-aware default buffer of max(2, altref_interval + 1) x target (a pyramid group's hidden alt-ref must fit ahead of the group's display points - the chain's two-frame default starves the frame after it to q = 255); cfg.base_q_idx is the first-pass probe quantizer; Vp9TwoPassFrame gains show_existing (one report entry per packet in decode order); MEASURED 99.8-99.9 % of the pool on every shape / format (pyramid, full segmentation on a scene cut, intra-only alt-refs, tile rows, 4:4:4 / 4:2:2 / 4:4:0, 10-bit 4:2:0 / 12-bit 4:2:2 / 10-bit 4:4:0) with every decoded packet within its budget; pinned by shape + format + contract tests and an env-gated dump (OXIDEAV_VP9_RC_DUMP_DIR) under which all ten streams decode byte-exact through the black-box reference decoder
- switchable interpolation filter election on sub-8x8 leaves and scaled-reference leaves - the two paths that kept EIGHTTAP under a SWITCHABLE frame filter now elect their §6.4.16 interp_filter: the sub-8x8 cell walk scores the whole per-blockIdx §8.5.2 prediction under the three §8.5.2.4 kernels (every cell shares the leaf's one coded kernel; the §8.5.2.1 averaged chroma vector is where the kernels differ on integer per-cell motion) and keeps the least SSE, and the §8.5.2.3 scaled leaves elect on every leaf including ZEROMV (a scaled phase is non-zero whenever xScale != 1 << 14, so zero motion is not kernel-invariant there); the resized chain codes interpolation_filter = SWITCHABLE on every P-frame (Vp9GopConfig::switchable_interp_filter now applies to encode_vp9_lossy_sequence_resized_with; the default resized entries elect, the round-452 fixture builder opts out and its bytes are unchanged); MEASURED on the resized mirror GOP: 8492 -> 8371 bytes at SSE 475937 -> 449230 (interp_filter counts [96, 125, 16] - SMOOTH wins most downscaled leaves), corpus 28102 -> 28090 bytes at equal-or-better PSNR on every sequence; pinned by a sub-8x8 election test (EIGHTTAP_SHARP coded on every divergent cell against a sharp-interpolated chroma target, writer counts == decoder counts, mirror-exact, strictly better SSE than the EIGHTTAP encode) and the resized mirror test (SWITCHABLE headers, live non-EIGHTTAP counts, SSE no worse than the fixed-kernel chain); both dumped streams decode byte-exact through the black-box reference decoder
- README - round 455 remaining-gaps list rewritten honestly (sub-8x8 / scaled leaves keep EIGHTTAP, node-estimate coefficient election, plain-chain two-pass) + test-count accuracy
- encode_adaptive_chain fuzz target - tenth harness: oracle-carrying round-trip over the round-455 adaptive lossy chain (encoder-side §8.4 backward adaptation, cost-elected §6.3 forward updates, per-leaf switchable interpolation filter) and the two-pass rate-controlled chain (first-pass statistics, VBV allocation) across fuzz-derived geometry / quantizer / budget / content: the emitted packets must decode to exactly the source frame count at the coded size, every two-pass frame within its reported budget (or at the q == 255 floor) - a desynchronised entropy mirror corrupts the next frame's §9.2 bool decode and surfaces as a decode error by construction; 2905 runs / 420 s clean locally, encode_gop_structures 6535 runs / 300 s clean on the new defaults; fuzz workflow roster comment refreshed
- format-matrix wrappers for the structured-GOP and resized entries - encode_vp9_lossy_sequence_with_444/_422/_440/_hbd/_hbd_422/_hbd_440 (a Vp9GopConfig over the structured GOP encoder at every §7.2 format: alt-ref pyramid, segmentation, tiles, intra-only alt-refs, entropy model, switchable filter - a native-u16 front end joins the 8-bit one) and encode_vp9_lossy_sequence_resized_444/_422/_440/_hbd/_hbd_422/_hbd_440 (the resized chain lifted off its 4:2:0 8-bit pin onto a format-generic core over pre-padded targets, with u8 / u16 front ends; samples above the declared depth reject); pinned by a 14-stream sweep decoding at the declared profile / bit depth / subsampling / per-frame dims at 39-42 dB, determinism + rejection tests, and an env-gated dump under which ALL FOURTEEN streams decode byte-exact through the black-box reference decoder (auto-scaling off for the resized ones); closes the README's first remaining-gap bullet
- encode_rate_control test - factor the GOP-shape tuple into a type alias (clippy type_complexity on the new integration suite)
- two-pass rate control - encode_vp9_lossy_sequence_rc_two_pass(frames, w, h, target_bytes_per_frame, vbv_bytes): pass one codes the sequence at the fixed probe quantizer 110 through the adaptive chain recording each frame's coded size (intra / inter cost) and motion activity (mean eighth-pel |mv| over inter MIs from the §6.4.4 field); pass two allocates target x frames in proportion to the first-pass sizes and codes under a leaky-bucket VBV model (buffer starts full, drains per coded frame, refills by the target; budget_i = min(allocation_i + unspent carry, level)), each frame bisected to its budget on the one-pass chain body now generalized over a per-frame budget policy (encode_sequence_lossy_rc_420_budgeted - the one-pass entry passes a constant and stays byte-identical, the staged r445 rc pin green); returns the packets + a per-frame Vp9TwoPassFrame report (first-pass bytes, motion activity, budget, coded bytes, landed q); MEASURED across static / translating / scene-cut / tight-budget GOPs: 99.9-100% of the sequence target (one-pass lands 98.5-99.6%) at +0.2..+6.8 dB PSNR over the one-pass chain (the pool moves to the keyframe and the scene-cut frame); pinned by rate-accuracy + VBV bookkeeping, first-pass statistic (scene cut > 2x its neighbour, zero motion on static GOPs), determinism + contract tests
- per-leaf switchable interpolation filter election - adaptive P-frames code §6.2.7 interpolation_filter = SWITCHABLE and every >= BLOCK_8X8 leaf with non-zero motion is predicted under the three §8.5.2.4 kernels (EIGHTTAP / EIGHTTAP_SMOOTH / EIGHTTAP_SHARP), keeping the least full-leaf SSE and coding it as §6.4.16 interp_filter under the §8.4-adapted interp_filter_probs (zero motion codes EIGHTTAP - phase 0 is the identity tap); the encoder's leaf predictors take the kernel index (every earlier caller passes EIGHTTAP); Vp9GopConfig::switchable_interp_filter (default on, the r452 fixture builders opt out) and the corpus test split the axis from the entropy model: MEASURED +0.01..+0.33 dB PSNR on every corpus sequence at 28253 -> 28102 bytes on top of the entropy win, all six streams black-box byte-exact through the reference decoder with the three kernels live; sub-8x8 cells and scaled-reference leaves keep EIGHTTAP; decoder-mirror suites (chained + every structured shape) now run with the switchable framing
- encoder-side entropy model - writer-side §8.4 backward adaptation + cost-elected §6.3 forward updates - every symbol writer counts its §9.3.4 events exactly as the decoder does (more_coefs errata rule, corner-inferred PARTITION_SPLIT, absent mv_hp counted), entropy_model::EntropyModel mirrors the decoder's FrameContext[4] banks (§6.2 setup_past_independence / reset_frame_context resets, §6.1.2 refresh_probs through the decoder's own §8.4.2-§8.4.4 code with §7.1.2 load_probs semantics + LastFrameType), and elect_forward_updates picks per-cell diff_update_prob / update_mv_prob transitions by integer-log2 bit cost (tree cells on the §8.4.2 node counts, coefficient slabs against their update_probs bit; the updated assembly kept only when strictly shorter, symbols never depend on the bank so the reconstruction is invariant); compressed-header writers code base->coding transitions (term_subexp encoder + reachable-delta search); every public lossy sequence entry (chain, 4:4:4/4:2:2/4:4:0/HBD, rc, structured, resized) now codes refresh_frame_context = 1 / frame_parallel_decoding_mode = 0 (ChainModel::Adaptive), new Vp9GopConfig::entropy_adaptation (default on) + encode_vp9_lossy_sequence_resized_with, the staged r441/445/448 fixture builders pin the pre-455 ChainModel::Chained framing; MEASURED 30495 -> 28253 bytes (-7.4%) across the six-shape encode corpus at bit-identical reconstruction, all six streams black-box byte-exact through the reference decoder (PSNR 29-44 dB vs source); pinned by per-frame decoder-mirror tests (writer counts == decoder counts cell for cell, encoder bank == decoder FrameContext[0] on the chain and on every structured shape - pyramid, all four segmentation modes, intra-only alt-refs, tile cols/rows), random-reachable-bank forward-update header round-trips, and the corpus measurement test

### Added

- encode_two_pass_structured fuzz target (round 458) - eleventh harness: oracle-carrying round-trip over the structured two-pass entries (every Vp9GopConfig axis incl. scene-cut keyframe placement, adaptive group length and the coefficient election, at 4:2:0 / 4:4:4 / 4:2:2 / 4:4:0 / 10-bit 4:2:0) across fuzz-derived geometry / budget / VBV / content with an optional mid-sequence scene cut: one report entry per packet, the first a keyframe, every display frame coded exactly once, every decoded packet within its budget (or at the q == 255 floor), show_existing_frame packets at most two bytes, the stream decoding to exactly the source frame count at the declared format; 1345 runs / 300 s clean locally, encode_gop_structures 2603 runs / 150 s and encode_adaptive_chain 818 runs / 150 s clean on the round-458 defaults (coefficient election + sub-8x8 / scaled-leaf filter election live); two tracked seeds; fuzz workflow roster refreshed
- encoder profiling + byte-identical speedups (round 458) - Criterion bench `benches/encode.rs` (structured GOP + plain chain at 96x64 x 6 frames, lossy keyframe at 256x160; `cargo bench --bench encode`) and a sampling profile of the chain encoder (predict_inter under the sub-pel refinement 22 %, the integer full search, the forward DCT's per-coefficient cos( ) call, the writer); four speedups, each provably output-preserving: (1) the forward DCT's cosines come from a per-size table holding the exact doubles the inline expression produced (the summation order unchanged); (2) the integer full search's SAD row loop bails out once the partial sum reaches the running best - a winner must be strictly below it, so no elected vector changes; (3) the §8.5.2.4 eight-tap horizontal pass reads its taps straight from the clipped reference row's slice whenever the window lies inside [0, lastX] (every interior column; the edge fallback keeps the per-tap Clip3), the tap products accumulated in the same order - pinned by an equivalence test against the generic form across every kernel, scaled / unscaled steps and blocks straddling every plane edge; (4) the sub-pel descent memoises each candidate vector's SAD (the descent revisits up to six of the previous neighbourhood per move and every coarser position per finer step); MEASURED (release, this machine): structured GOP 220.5 -> 155.6 ms (27.2 -> 38.6 fps, -29 %), chain 191.7 -> 138.5 ms (31.3 -> 43.3 fps, -28 %), keyframe 256x160 49.1 -> 29.5 ms (20.4 -> 33.9 fps, -40 %); every corpus byte count and every staged-fixture identity pin unchanged
- coefficient election over the §6.4.24 token tree (round 458) - `rdoq` module + `Vp9GopConfig::coefficient_rdo` (default on; every adaptive-framing entry - chain, rc, two-pass, structured, resized, registry): each trial quantisation re-elects its levels rate-distortion-wise against the frame's OWN loaded §6.1.2 bank - the per-cell token-tree costs (internal nodes above TWO through the §9.3.2 pareto( ) tail exactly as build_token_probs derives them), extra magnitude bits under CAT_PROBS, sign, and the more_coefs flags, with the §9.3.2 context replayed over the block's own already-elected tokens - against pixel-domain distortion measured through the crate's own §8.7 inverse transform (per transform size and coefficient position a unit-coefficient gain is taken from inverse_transform_2d once, so the trade-off is paid in reconstruction-error units); two deterministic greedy passes: tail trimming (a trailing ONE and the zero run before it go when their bits outweigh the distortion they save, the closing flag moving up) and level reduction in scan order (a level one lower, ONE -> ZERO also suppressing the next position's more_coefs flag per the §6.4.24 check_eob latch); lambda = qstep_px² / 16 per bit (measured against 1/8 and 1/32 on the corpus); integer-only arithmetic, the decoder untouched (elected levels are ordinary tokens through the existing writer and reconstruction mirror); MEASURED on the six-shape corpus against a five-quantizer plain scalar-quantiser RD curve: every sequence lands above the curve, +0.19..+1.87 dB at equal bytes, mean +4.8 % fewer bytes at equal PSNR; the two-pass chain at equal target: static 38.14 -> 38.81 dB, moving 41.70 -> 42.06, scene cut 36.09 -> 36.69; pinned by the RD-curve test (on-or-above on every corpus sequence), unit tests (measured gains, bank-following costs, a marginal trailing ONE trimmed while exact levels stay), and the corpus dump byte-exact through the black-box reference decoder; the entropy-model identity pin holds the election off (it reads the bank, so the reconstruction legitimately moves with it); fixed-quantiser fixture builders (ChainModel::Chained, entropy_adaptation = false) are byte-unchanged
- scene-cut keyframe placement + adaptive alt-ref group length from first-pass statistics (round 458) - the structured two-pass entries plan their second pass: `Vp9GopConfig::scene_cut_keyframes` (default on) places a keyframe on every display frame whose first-pass inter cost spikes (above twice the median inter cost, or at least two thirds of the first keyframe's intra cost) AND is confirmed by a probe keyframe encode at the first-pass quantizer to be at least three quarters of its intra cost - inter prediction bought nothing worth its syntax, so the keyframe is as cheap and resets the prediction chain (every slot refreshed, roles restarted, a new alt-ref segment); `Vp9GopConfig::adaptive_group_length` (default on) halves an alt-ref group whose frames' mean first-pass motion activity exceeds 48 eighth-pel until it fits or is a lone P-frame (a distant alt-ref predicts fast motion poorly; not applied under intra_only_altref, whose period is a refresh choice; hidden intra-only alt-refs are never cut candidates - their first-pass size is an intra cost); the structured encoder now codes an explicit GopPlan packet sequence (`encode_packets` over PlannedPacket { Key | HiddenAltref | Shown | ShowExisting }, `encode_with` = the structure's default plan, byte-identical), mid-stream keyframes through the same commit path as the first; the second-pass allocation weights the planned packets with their first-pass cost (a placed keyframe: its probe intra cost); Vp9TwoPassFrame gains keyframe + frame (the display frame each packet codes); MEASURED at equal bytes vs planning off: seg-full scene-cut GOP 34.37 -> 35.03 dB (keyframe placed on the cut frame), pyramid 37.23 -> 37.52, tile-rows 36.77 -> 37.81, 4:4:4 32.60 -> 33.01, 4:2:2 34.18 -> 34.24, 4:4:0 34.53 -> 35.10, 10-bit 4:2:0 42.09 -> 42.35, 12-bit 4:2:2 41.55 -> 42.00, intra-only and static GOPs byte-identical (planning is a no-op there); pinned by the A/B in the shapes test, a static-GOP no-op test, and the env-gated dump with every stream byte-exact through the black-box reference decoder
- two-pass rate control over the structured GOP + the format matrix (round 458) - `encode_vp9_lossy_sequence_rc_two_pass_with(frames, w, h, cfg, target_bytes_per_frame, vbv_bytes)` and `_444/_422/_440/_hbd/_hbd_422/_hbd_440`: every Vp9GopConfig axis (alt-ref pyramid, segmentation, tiles, intra-only alt-refs, entropy model, switchable filter) at every §7.2 format under the two-pass allocation; the StructuredGopEncoder is refactored into a probe / commit split (prepare_keyframe / prepare_pframe / prepare_intra_only_arf code + finish a frame at a quantizer against the read-only encoder state and return a PendingFrame carrying every persistent side effect - slot refresh, §7.2.6 field, §6.4.14 map, §7.2.8 baseline, §7.2.10 table, §6.1.2 bank products - that commit applies; EntropyModel gains the mutation-free begin_frame_view / bank_after_begin twins) and a per-packet quantizer policy (`encode_with`: FrameQ::Fixed or FrameQ::Budget bisected through bisect_q exactly as the chain), the fixed-quantizer path byte-identical (the five staged round-452 fixtures still pin their bytes); pass one records per-packet size + motion activity (`StructuredGopEncoder::stats`, the packet sequence depends only on frame count and structure so first-pass statistics index the second pass one for one), pass two allocates the pool less the show_existing_frame packets proportionally to first-pass size (hidden alt-refs draw their share) under the VBV model with refills at DISPLAY points (a shown frame or the show_existing_frame packet presenting the alt-ref) and a structure-aware default buffer of max(2, altref_interval + 1) x target (a pyramid group's hidden alt-ref must fit ahead of the group's display points - the chain's two-frame default starves the frame after it to q = 255); cfg.base_q_idx is the first-pass probe quantizer; Vp9TwoPassFrame gains show_existing (one report entry per packet in decode order); MEASURED 99.8-99.9 % of the pool on every shape / format (pyramid, full segmentation on a scene cut, intra-only alt-refs, tile rows, 4:4:4 / 4:2:2 / 4:4:0, 10-bit 4:2:0 / 12-bit 4:2:2 / 10-bit 4:4:0) with every decoded packet within its budget; pinned by shape + format + contract tests and an env-gated dump under which all ten streams decode byte-exact through the black-box reference decoder
- switchable interpolation filter election on sub-8x8 leaves and scaled-reference leaves (round 458) - the two paths that kept EIGHTTAP under a SWITCHABLE frame filter now elect their §6.4.16 interp_filter: the sub-8x8 cell walk scores the whole per-blockIdx §8.5.2 prediction (every cell shares the leaf's one coded kernel; the §8.5.2.1 averaged chroma vector is where the kernels differ on integer per-cell motion) under the three §8.5.2.4 kernels and keeps the least SSE, and the §8.5.2.3 scaled leaves elect on every leaf including ZEROMV (a scaled phase is non-zero whenever xScale != 1 << 14, so zero motion is not kernel-invariant there); the resized chain codes interpolation_filter = SWITCHABLE on every P-frame (`Vp9GopConfig::switchable_interp_filter` now applies to `encode_vp9_lossy_sequence_resized_with`; the default resized entries elect, the round-452 fixture builder opts out and its bytes are unchanged); MEASURED on the resized mirror GOP: 8492 -> 8371 bytes at SSE 475937 -> 449230 (interp_filter counts [96, 125, 16] - SMOOTH wins most downscaled leaves), corpus 28102 -> 28090 bytes at equal-or-better PSNR on every sequence; pinned by a sub-8x8 election test (EIGHTTAP_SHARP coded on every divergent cell against a sharp-interpolated chroma target, writer counts == decoder counts, mirror-exact, strictly better SSE than the EIGHTTAP encode) and the resized mirror test (SWITCHABLE headers, live non-EIGHTTAP counts, SSE no worse than the fixed-kernel chain); both dumped streams decode byte-exact through the black-box reference decoder
- encode_adaptive_chain fuzz target (round 455) - tenth harness: oracle-carrying round-trip over the adaptive lossy chain (encoder-side §8.4 backward adaptation, cost-elected §6.3 forward updates, per-leaf switchable interpolation filter) and the two-pass rate-controlled chain (first-pass statistics, VBV allocation) across fuzz-derived geometry / quantizer / budget / content: the emitted packets must decode to exactly the source frame count at the coded size, every two-pass frame within its reported budget (or at the q == 255 floor); a desynchronised entropy mirror corrupts the next frame's §9.2 bool decode and surfaces as a decode error by construction; 2905 runs / 420 s clean locally, encode_gop_structures 6535 runs / 300 s clean on the new defaults; fuzz workflow roster refreshed
- format-matrix wrappers for the structured-GOP and resized entries (round 455) - `encode_vp9_lossy_sequence_with_444/_422/_440/_hbd/_hbd_422/_hbd_440` (a Vp9GopConfig over the structured GOP encoder at every §7.2 format: alt-ref pyramid, segmentation, tiles, intra-only alt-refs, entropy model, switchable filter) and `encode_vp9_lossy_sequence_resized_444/_422/_440/_hbd/_hbd_422/_hbd_440` (the resized chain generalized over LossyFormat with native-u16 front ends; out-of-range samples for the declared depth reject); pinned by a 14-stream sweep decoding at the declared profile / bit depth / subsampling / per-frame dims at 39-42 dB, determinism + rejection tests, and an env-gated dump under which all fourteen streams decode byte-exact through the black-box reference decoder
- two-pass rate control (round 455) - `encode_vp9_lossy_sequence_rc_two_pass(frames, w, h, target_bytes_per_frame, vbv_bytes)`: pass one codes the sequence at the fixed probe quantizer 110 through the adaptive chain recording each frame's coded size (intra / inter cost) and motion activity (mean eighth-pel |mv| over inter MIs from the §6.4.4 field); pass two allocates target x frames in proportion to the first-pass sizes and codes under a leaky-bucket VBV model (buffer starts full, drains per coded frame, refills by the target; budget_i = min(allocation_i + unspent carry, level)), each frame bisected to its budget on the one-pass chain body now generalized over a per-frame budget policy (`encode_sequence_lossy_rc_420_budgeted`; the one-pass entry passes a constant and stays byte-identical); returns the packets + per-frame `Vp9TwoPassFrame` report (first-pass bytes, motion activity, budget, coded bytes, landed q); MEASURED across static / translating / scene-cut / tight-budget GOPs: 99.9-100 % of the sequence target (one-pass 98.5-99.6 %) at +0.2..+6.8 dB PSNR over one-pass; pinned by rate-accuracy + VBV bookkeeping + first-pass statistic + contract tests
- per-leaf switchable interpolation filter election (round 455) - adaptive P-frames code §6.2.7 interpolation_filter = SWITCHABLE and every >= BLOCK_8X8 leaf with non-zero motion is predicted under the three §8.5.2.4 kernels (EIGHTTAP / EIGHTTAP_SMOOTH / EIGHTTAP_SHARP), keeping the least full-leaf SSE and coding it as §6.4.16 interp_filter under the §8.4-adapted interp_filter_probs (zero motion codes EIGHTTAP: phase 0 is the identity tap); `Vp9GopConfig::switchable_interp_filter` (default on) and the corpus test measure the axis separately from the entropy model: +0.01..+0.33 dB PSNR on every corpus sequence at 28253 -> 28102 bytes, all six streams black-box byte-exact through the reference decoder with the three kernels live; sub-8x8 cells and scaled-reference leaves keep EIGHTTAP
...
- README + CHANGELOG - 4:4:0 registry labels are live (core 0.1.35), the pending-label prose replaced by the twelve-format registry truth
- 4:4:0 framework labels - core 0.1.35 consumer wave
- encode_gop_structures fuzz harness - resized arm derives its half-size twin as ceil(w / 2), not floor - CI crash 73f3c7e7 (bytes fc fc fc 3a 0a f6 fc 26 -> 33x20 on the resized arm) took floor(33 / 2) = 16 as the twin, a 2x-PLUS downscale the $5 scaling bounds forbid, so the encoder's Unsupported was the CORRECT rejection of an invalid harness input, not an encoder defect; the derivation now stays inside the contract (expect only fires on a real defect) and a regression test pins both halves - the floor twin rejects, the ceil twin encodes/decodes at its declared sizes in both directions
- README test-count accuracy (1283 tests after round 452)
- intra-only frame emission - Vp9GopConfig::intra_only_altref codes each alt-ref group's hidden frame as a $6.2 intra-only frame (frame_type = NON_KEY, show_frame = 0, intra_only = 1, reset_frame_context = 3, single-slot refresh) - a mid-GOP refresh point without a keyframe in the display stream, surfacing through its show_existing_frame packet; the keyframe planner + tree encoder accept the $6.2 FrameIsIntra predicate (key frame or hidden intra-only - mode_info( ) dispatches on FrameIsIntra so both code the identical intra body), and the encoder mirrors the decoder's $7.2 setup_past_independence( ): $7.2.8 delta baseline + $7.2.10 feature table restart (update_data = 1), PrevSegmentIds replaced by the frame's own update_map = 1 walk ($8.1 step 3), next decoded frame with the prev field absent ($7.2.6 (c)/(e)); pinned by a decoder-mirror with the full segmentation emission stacked on, header pins re-parsed from the packet bytes, the config-entry contract, and the stream locally byte-exact through the black-box reference decoder; fifth fixture package intra-only-emitted-gop joins the staging generator (all five locally byte-exact); README remaining-gaps list updated
- four round-452 fixture packages built + wired (staging pending) - altref-pyramid-gop (the corpus's first encoder-minted hidden alt-ref + show_existing_frame stream, three-slot [LAST, GOLDEN, ALTREF] election), seg-emitted-full-gop (all four SEG_LVL_* features emitted with fitted tree/pred probs, temporal map updates, persistent feature table - the existing seg-features fixtures are black-box encodes, this pins the writer side), tiles-2col-encoded-gop (the first SELF-ENCODED multi-tile-column stream - the $9.2.4 tile-parallel decoder's consumer-side twin), resized-encoded-gop (the first PIXEL-ACCURATE self-encoded $8.5.2.3 stream - the r409 scaled-reference fixture is all-skip ZEROMV); each generated deterministically through the PUBLIC entries (stage_round_452_fixtures_when_requested under OXIDEAV_VP9_STAGE_DIR, input.ivf + the crate's own decode as expected.yuv) and locally verified byte-exact against the black-box reference decoder (timestamp passthrough; auto-scale disabled for the resized stream); in-crate wiring complete - builder byte-determinism pins, staging generator, docs-gated staged-bytes + expected.yuv identity tests
- scaled-reference ENCODE + structured-GOP fuzz target + README encoder rewrite - encode_vp9_lossy_sequence_resized mints $8.5.2.3 mid-stream coded-size changes pixel-accurately: predict_inter_leaf2 generalized over distinct reference dimensions (xScale = (RefFrameWidth << 14) / FrameWidth; equal sizes reduce to the unscaled path bit-for-bit), the partition tree planned against a whole-frame ZEROMV scaled virtual reference, each leaf electing ZEROMV vs a log-diamond NEWMV descent with every candidate scored by the decoder's own $8.5.2.3 prediction (parity-snapped vs $6.5.12 BestMv, $6.4.16-mapped to NEARESTMV/NEARMV), then the ordinary per-leaf tx + skip elections; $7.2.6 (b) modeled (UsePrevFrameMvs = 0 across a resize) and the $5 ratio bounds enforced (2x down / 16x up per axis) - closes the r409 tooling-gap asymmetry (that fixture was all-skip ZEROMV; the crate's own encoder now codes real content across a resize); pinned by a 4-frame 2x-down / 4/3-up / back-to-full GOP decoding at its declared sizes in the quantizer's PSNR regime, byte-determinism, contract + rejections, and the dumped stream locally byte-exact through the black-box reference decoder (timestamp passthrough + auto-scale disabled). NINTH fuzz target encode_gop_structures: fuzz-derived Vp9GopConfig axes (pyramid interval, all four segmentation modes, tile rows) + a resized arm, oracle = decode to exactly the source frame count of shown frames at the declared sizes; workflow roster refreshed. README encoder section rewritten for r452 (structured GOP + resized entries, honest remaining-gaps list: no writer-side $8.4 backward adaptation, one-pass RC only, no intra-only emission, frame-level EIGHTTAP)
- tile columns + tile rows on the write side - the encoder mints the streams the $9.2.4 tile-parallel decoder consumes - Vp9GopConfig gains tile_cols_log2 / tile_rows_log2 (validated against the $6.2.14 min/max derivation at write time) and the structured-GOP pipeline is tile-correct encoder-side: intra prediction derives $6.4.4 AvailL = MiCol > MiColStart (the reconstruction-plane predictor severs the left neighbour at a tile's left edge exactly as the decoder's per-tile walk does; AvailU stays frame-level, the above strips carry across tile rows), and every $6.5 candidate scan in the P-frame planner - leaf evaluation, compound second-list re-snap, NEARESTMV/NEARMV mapping, sub-8x8 cell walk - clamps its column window to the leaf's tile extents via the $6.4 get_tile_offset( ) bracket (identity on single-tile frames, so every existing entry is byte-unchanged); pinned by decoder-mirrors on a 2-tile-column 512-px GOP (pyramid framing) and a 2-tile-row GOP stacked with the full segmentation emission, a $9.2.4 tile-parallel parity sweep (the self-encoded multi-column stream decodes byte-identically through decode_vp9_sequence_with at a 4-thread budget), config-entry contract + rejection tests, and both streams locally byte-exact through the black-box reference decoder
- $6.2.11 segmentation feature emission - all four SEG_LVL_* features config-driven, plus a $6.4.14 PrevSegmentIds decoder fix - new Vp9GopConfig (#[non_exhaustive]) + encode_vp9_lossy_sequence_with generalize the structured GOP encoder over altref_interval and a Vp9Segmentation mode: AdaptiveQuant (four activity classes by per-leaf luma MAD carrying SEG_LVL_ALT_Q deltas [-16,-6,+4,+12] with per-segment-qindex-aware quantization at every trial/replay site, plus a per-frame per-segment SEG_LVL_ALT_L election swept by full-$8.8-filter SSE), StaticSkip (leaves whose co-located LAST prediction's quantized residual is all zero code on the SEG_LVL_SKIP + SEG_LVL_REF_FRAME=LAST segment - $6.4.8/$6.4.13/$6.4.16/$6.4.17 read no skip/is_inter/ref_frame/inter_mode syntax and $6.4.21 no residual; the $6.4.16 MiSize >= BLOCK_8X8 bound honored), Full (both); every decoded frame codes update_map=1 with per-frame FITTED segmentation_tree_probs (probe encode collects $9.3 segment_tree node counts) + temporal_update=1 whenever a $6.4.14 predictor exists (fitted pred_prob), and the $7.2.10 persistent feature table (update_data only when the elected table moved). Per-segment lossless documented as nonexistent in VP9 ($6.2.9 derives Lossless from base_q_idx alone). DECODER FIX: PrevSegmentIds was invalidated by a hidden predecessor - the show_frame gate belongs to $7.2.6 (c) UsePrevFrameMvs ONLY, $8.1 step 3 refreshes the map after every decoded update_map=1 frame; found by this round's decoder-mirror (first stream combining a hidden alt-ref with a temporal map update), fix threads prev_shown for the $7.2.6 gate alone. Pinned by decoder-mirrors over all three modes on the pyramid GOP, header + coded-map pins, config-entry public contract, and three black-box dump cases (seg-full/seg-aq/seg-skip) locally byte-exact through the black-box reference decoder; also clears the clippy-1.98 toolchain drift (needless_late_init in bool_coder, manual_slice_fill in mode_info)
- alt-ref pyramid GOP encoder - hidden alt-refs + show_existing_frame + three-slot [LAST, GOLDEN, ALTREF] election - encode_vp9_lossy_sequence_altref codes groups of altref_interval display frames as a hidden (show_frame = 0) P-frame of the group's last source frame into a free $8.10 slot, the group's earlier frames as shown P-frames over ref_frame_idx = [last, golden, alt] with ref_frame_sign_bias = [0, 0, 1] (a genuinely future alt-ref), and a one-byte $6.2 show_existing_frame packet displaying the alt-ref at its display position (the alt-ref slot becomes LAST, the freed slot hosts the next group's alt-ref, the keyframe stays the long-term GOLDEN); the inter tree encoder gains a distinct third reference (encode_pframe_lossy_tree_motion_with_state_alt): every leaf sweeps single LAST / GOLDEN / ALTREF (ZEROMV + searched sub-pel NEWMV each) and the [LAST, ALTREF] $6.3.18 compound pair over the true alt-ref planes, the two-slot entries passing None stay byte-identical; $7.2.6 modeled per compute_image_size( ) - the frame after a hidden alt-ref codes with the prev field absent, a show_existing_frame packet invokes neither compute_image_size( ) nor the $6.1.2 PrevMvs save; pinned by a decoder-mirror over every shown frame (the alt-ref surfacing at its show_existing_frame position), packet-structure + slot-rotation pins, a cross-fade A/B (midpoint frame under half the two-slot chain's bytes), the interval-1 degenerate chain, rejections, and a gop-altref-pyramid black-box dump case - all 24 dumped streams locally byte-exact through the black-box reference decoder
- decode_tile_parallel_diff fuzz target - a total differential oracle over the tile-parallel path - the eighth harness decodes every input twice (serial walk vs 4-thread tile-column fan-out) and requires the identical outcome: byte-for-byte equal shown frames on Ok, the equal error value on Err (tiles are independent, so the failing-tile set is schedule-invariant and the parallel path surfaces the lowest raster-order tile error - the one serial hits first), any Ok/Err split a bug by construction; single-column inputs pin the effective_workers clamp, tile_cols_log2 > 0 headers drive the real fan-out / worker chunking / column-range merges; fuzz workflow roster comment refreshed
- tile-parallel decode - the $9.2.4 multi-coder path, ExecutionContext-governed - multi-tile-column frames decode their tile columns concurrently with oxideav_core::ExecutionContext as the single threading authority (fan-out = effective_workers(tileCols), serial until told otherwise): new decode_vp9_sequence_with + Vp9SequenceDecoder::set_execution_context public surfaces and the framework Vp9Decoder honoring the registry set_execution_context hook (budget preserved across reset); the parallel unit is the tile COLUMN (rows of it in order) - $6.4.4 AvailL = MiCol > MiColStart severs left neighbours, SB64-aligned boundaries bound every $8.5.1 edge read, $6.5 candidate scans clamp to the tile window - each worker decodes its columns into private full-extent state and disjoint MI column ranges merge back deterministically (Plane::copy_cols_from + Vp9FrameState::merge_cols_from + FrameCounts::merge_from, the latter two exhaustively destructured); both paths run tiles through one extracted decode_one_tile body so serial and parallel cannot drift; byte-identical output pinned by sample + $9.3.4-count-bank equality on tiled keyframes / uneven columns / NEWMV-straddling P-frames at 2/3/8-thread budgets, a whole-corpus 4-thread parity sweep, and a registry parity + reset-persistence sweep; measured (release, 1080p 4-column black-box-encoded streams): 3.10x at 4 threads / 1.78x at 2 on coefficient-heavy content, 1.31x on easy content (the $8.8 filter stays serial); env-gated OXIDEAV_VP9_TILE_BENCH_IVF timing probe committed (asserts byte-identity, never gates CI)
- 4:4:0 public entries + two fixture packages (staging pending) - the last $7.2.2 geometry gets its dedicated public surface: encode_vp9_lossy_440 / encode_vp9_lossy_hbd_440 keyframes and encode_vp9_lossy_sequence_440 / encode_vp9_lossy_sequence_hbd_440 chain-framed GOPs (ssx=0/ssy=1 - profile 1 at 8-bit, profile 3 at 10/12-bit, $8.5.2.1 row-only chroma MV rounding), thin wrappers over the subsampling-generic lossy pipeline the 9-format decoder-mirror sweep pins; public-contract tests + four black-box dump cases (all 23 dumped streams locally byte-exact through the black-box reference decoder); two fixture packages built + wired presence-gated: lossy-440-gop (corpus-first SELF-ENCODED 4:4:0, writer-side pin for the geometry) and lossy-hbd12-440-gop (profile 3 / 12-bit / 4:4:0 - the only $7.2 matrix corner with no corpus fixture at all), deterministic staging generator + shape pins + docs-gated staged-bytes identity + header-flags geometry pin; registry surface unchanged (no framework 4:4:0 pixel-format label - explicit Unsupported kept at the frame-conversion boundary)
- black-box dump harness extended to the r445 default paths - dump_matrix_streams_for_black_box_validation gains gop-420-8bit-default (the promoted default lossy sequence), gop-lossless-chained (the default lossless entry: chain framing + skip-elected keyframe), and gop-420-8bit-rc (the rate-controlled chain); local sweep: all 19 dumped streams (10 keyframe matrix formats + 9 GOP classes) decode byte-exact through the black-box reference decoder against the crate's own decode
- README refresh for round 445 - chained-as-default promotion (default sequence entries on $7.2.6 chain framing with r441 elections + new lossless keyframe skip election; _error_resilient opt-outs with frozen pre-445 bytes; _chained aliases), framework registry section (Vp9Encoder byte-identical to the batch entries, Vp9Decoder over the incremental Vp9SequenceDecoder with per-packet Annex B split, whole-corpus registry sweep), r441 packages now staged, three r445 packages built/verified/wired awaiting staging, RC chain framing + budget-guarded delta election, 1250+ tests
- three round-445 fixture packages built + wired (staging pending) - new black-box byte-exact stream classes on the chained defaults: lossless-chained-gop (corpus-first chain-framed LOSSLESS stream, $7.2.6 UsePrevFrameMvs=1 over $8.7.2 WHT residuals, first skip-elected lossless keyframe, via the now-default encode_vp9_lossless_sequence), lossy-hbd12-422-gop (first self-encoded stream at the $7.2 matrix's deepest corner: profile 3 / 12-bit / 4:2:2 CAT6 tokens through the chained pipeline), lossy-rc-gop (corpus-first rate-controlled stream: bisected per-frame q on chain framing + budget-guarded $6.2.8 delta election); deterministic staging generator (also emits the crate decode in expected.yuv packing for the black-box compare), each package locally verified byte-exact between the black-box reference decode and decode_vp9_sequence; shape pins + docs-gated staged-bytes identity + presence-gated corpus-sweep entries
- r441 elections threaded onto every chained public entry - the RC chain (encode_vp9_lossy_sequence_rc) rides $7.2.6 chain framing (prev-frame-MV model seeded from the keyframe's $6.4.4 field, keyframe skip election inside the quantizer bisection, $6.2.8 delta election under the byte budget via finish_frame_with_filter_deltas_budget: overflow falls back to the update-free frame filtering on the $7.2.8 persisted baseline, persistent state unmoved on both sides, budget guarantee preserved); the chain-framed default lossless keyframe elects skip (encode_keyframe_lossless_elect_skip, two-pass token-cache mirror of the lossy election: all-zero-WHT MIs = exact DC prediction code skip=1, strict rate win at byte-exact lossless reconstruction); classic _error_resilient entries keep their staged-fixture-pinned non-electing writers; pinned by chain-framing header sweeps, dual-budget delta sweeps, flat-strict-win + noise-byte-identical keyframe pins, and end-to-end lossless round-trips
- framework registry integration - Vp9Decoder/Vp9Encoder + make_decoder/make_encoder, the registry Encoder riding the $7.2.6 chained default - register() installs a decode+encode registration (lossy+lossless, nine $7.2 matrix pixel formats, VP90/VP09/V_VP9 tag claims, q/lossless encoder options schema); Encoder packet bytes pinned byte-identical to the public batch entries at 4:2:0/4:4:4/HBD/lossless; Decoder is the new incremental Vp9SequenceDecoder (decode_vp9_sequence is now a thin loop over it, 43-fixture corpus-pinned) with per-packet Annex B split so superframes/hidden alt-refs/show_existing_frame work at the registry surface; chained sequence engines gain stateful push forms (LossyGopEncoder/LosslessGopEncoder420, batch entries thin loops, staged-fixture identity pins unchanged); whole-corpus registry sweep + stride/reset/pts/keyframe-flag/rejection tests; fuzz lockfile re-pinned to published core 0.1.34
- chained-as-default - the $7.2.6 chain framing becomes the public GOP path - encode_vp9_lossless_sequence / encode_vp9_lossy_sequence now emit shown non-error-resilient P-frames (decode-side UsePrevFrameMvs derivation = 1, prev-frame motion field threaded into every $6.5 predictor derivation, [LAST, ALTREF] compound election live in the ordinary shown GOP, r441 keyframe-skip + $6.2.8 delta elections on the lossy default with the $7.2.8 persistent baseline); the classic framing becomes the explicit opt-out (_error_resilient entries, bytes FROZEN as the exact pre-445 output - the staged odd-dims-59x37 / lossy-filtered-gop fixtures pin them and their builders now name the opt-out); _chained entries stay as byte-identical aliases; byte-stability contract documented on every entry; pinned by chain-framing header sweeps, alias identity, opt-out ER sweeps, dual-framing lossless byte-exactness, and the unchanged staged-fixture identity tests
- README refresh for round 441 - lossy format matrix complete (keyframes + chain-framed sequences at profiles 1/2/3, 10/12-bit, 4:4:4/4:2:2, all black-box byte-exact), keyframe skip election + $6.2.8 loop-filter delta election with $7.2.8 persistence threading, three fixture packages built/verified/wired awaiting staging, encode_lossy_matrix fuzz target, 1230+ tests
- encode_lossy_matrix fuzz target + in-CI deterministic smoke - third oracle-carrying harness: fuzz-derived (format over all seven public-entry $7.2 formats, w/h 1..=64, base_q_idx, cycled bit-depth-masked content) through the matching matrix keyframe entry with the stream-the-encoder-emitted-MUST-decode oracle (full $8.1 chain incl. elected $8.8 filter + skip-elected leaves); 28-case LCG-deterministic smoke of the same body runs in standard CI; fuzz workflow header comment refreshed from the stale two-target roster to the actual seven
- three round-441 fixture packages built + wired (staging pending) - lossy-444-gop (first self-encoded non-4:2:0 lossy stream, profile-1 chain framing), lossy-hbd10-gop (first self-encoded HBD lossy stream, profile 2 10-bit), lossy-lf-deltas-gop (first stream with a loop_filter_delta_update=1 frame: mode-delta update mid-GOP, a later frame filters on the $7.2.8 persisted value update-free, so the byte-exact sweep pins the persistence fold); deterministic builders + staging generator (OXIDEAV_VP9_STAGE_DIR), behavior pins, docs-gated staged-bytes identity, presence-gated corpus-sweep entries, and the update-then-persist header-shape regeneration guard; each package locally verified byte-exact between the black-box reference decode and this crate's own decode
- loop-filter delta election on the chain framing - elect_lf_deltas runs bounded coordinate descent (6 axes x <=11 candidates, full $8.8 replays) over the $6.2.8/$8.8.1 ref_deltas[4] + mode_deltas[2] after the (level, sharpness) election, accepting only strict SSE wins since a moved slot costs 1+7 bits; the writer codes exactly the slots that moved off the $7.2.8 persistent baseline and threads that baseline across the chain (LfDeltaState - keyframe/ER resets per $7.2, coded updates persist), with elect_filter_params_at/filter_reconstruction_with_deltas taking the resolved arrays; measured: mixed static/moving probe elects LAST +16 / ZEROMV-class -16 (SSE 350803->349178), public chained GOP codes mode_deltas[0]=+8 mid-GOP with the successor frame filtering on the persisted value update-free; pinned by a decoder-mirror at hand-coded deltas PLUS a no-update successor through decode_vp9_sequence (a $7.2.8 model mismatch desyncs), election never-regress/strict-win/determinism properties, and black-box byte-exact validation of the delta-electing GOP incl. persistence; classic paths byte-stable
- keyframe skip election - all-zero-residual leaves code skip=1 ($6.4.2 drops the residual syntax; a non-skip all-zero block still pays the $6.4.24 EOB token per coded block), a strict rate win at bit-exact identical reconstruction since $8.6.2 adds the zero residual either way and the keyframe's $6.4.6 read_tx_size(1) keeps coding the planned transform (unlike the inter !skip gate) - two-pass: the decoder-mirror encode caches every block's tokens + folds per-leaf all-zero flags, elected leaves flip in a second plan and the frame re-assembles draining the cache; wired into the standalone lossy keyframe APIs (all formats) + chain-framing sequences, classic paths byte-stable (staged-fixture pins green); measured 59->36 B (-39%) on a flat 256x192 frame, 704/768 MIs elected; pinned by identical-recon/strict-shrink/mirror/dense-noop/chained-vs-classic tests + black-box byte-exact skip-elected stream
- factor the sequence-matrix test's encoder-callback type into a type alias (clippy type_complexity on the new integration suite)
- lossy sequence encoding across the full $7.2 format matrix - encode_vp9_lossy_sequence_444/_422/_hbd/_hbd_422 on the $7.2.6 chain framing: the two 4:2:0 GOP bodies collapse into one format-generic encode_sequence_lossy_planes core (chained flag selects the framing, lossless_pframe_header_fmt threads the $6.2 format through every chroma/quantizer/$8.5.2 derivation), prev-MV modeling + compound election + per-frame $8.8 filter election live at every format; classic + chained 4:2:0 paths are byte-identical wrappers (staged lossy-filtered-gop identity green); pinned by a 5-format translating-GOP sweep through decode_vp9_sequence with drift-catching distortion bounds + determinism, and black-box validation: all five GOP streams byte-exact through the reference decoder
- lossy keyframe encoding across the full $7.2 format matrix - encode_vp9_lossy_444/_422 (8-bit profile 1) + encode_vp9_lossy_hbd/_hbd_422 (10/12-bit profiles 2/3): LossyFormat derives the $7.2 profile from the depth/subsampling pair and the $6.2.2 color_config plumbing + plane unpacking is the only new surface (planner, decoder-mirror tree encoder, $8.8 election were already format-generic); pinned by a 9-format decoder-mirror sweep incl. 4:4:0 and partial-MI geometries, per-depth distortion-regime bounds, rejection sweeps, and black-box validation (all nine matrix streams byte-exact through the reference decoder via the env-gated dump harness); 4:2:0 paths are byte-identical wrappers, staged-fixture identity tests green
- README refresh for round 434 - tile-rows fixture consumed (43/43 corpus, ledger closed), multi-tile tree assemblers, $7.2.6 UsePrevFrameMvs writer model + chained lossless/lossy sequence encoders (compound live in shown GOPs), chained-sequence fuzz target, 1210+ unit tests
- encode_vp9_lossy_sequence_chained - the lossy GOP on the $7.2.6 chain model with compound LIVE in ordinary shown GOPs: encode_pframe_lossy_tree_motion_with_state gains prev_frame_mvs, feeding the previous frame's $6.4.4 motion field into every $6.5 predictor derivation of the search (leaf candidate evaluation, compound second-list re-snap, NEARESTMV/NEARMV mode mapping, sub-8x8 cell walk) plus the block writer; a non-error-resilient frame keeps its CODED sign biases, so the asymmetric-ALTREF bias that was dead under $7.2 setup_past_independence on the error-resilient chain now genuinely admits [LAST, ALTREF] - the r418 hidden-predecessor restriction dissolves, pinned by a cross-fade-midpoint GOP electing compound leaves on the writer's own per-MI state with every frame shown, decoding midpoint-accurate; classic lossy/rc paths pass no prev field and stay byte-identical, staged-fixture identity tests green
- encode_chained_sequence fuzz target + in-CI deterministic smoke - second oracle-carrying harness: fuzz-derived (w,h 1..=48, 2..=4 frames, cycled shifted content) through encode_vp9_lossless_sequence_chained -> decode_vp9_sequence with the FULL lossless byte-exact oracle (stronger than must-decode: a prev-field model mismatch desyncs the $6.5 predictors and surfaces as byte divergence); 24-case LCG-deterministic smoke of the same body runs in standard CI across arbitrary partial-MI geometries; fuzz harness builds clean against the new API
- encode_vp9_lossless_sequence_chained - lossless video on non-error-resilient chain framing, the first production consumer of the $7.2.6 UsePrevFrameMvs writer model: shown non-ER P-frames thread each frame's $6.4.4 write-back into BOTH the next frame's motion-search predictor derivation and its block writer (encode_pframe_lossless_motion_prev; PrevMotionField::after_intra_frame pinned equal to the keyframe assembler's actual state); persistent motion the spatial neighbours mispredict codes NEARESTMV/NEARMV via the $6.5.10 prev candidate instead of NEWMV diffs - 1829 B vs 1845 B P-frame total on a 64x128 banded-motion probe (velocities +2/-2/+4/-4 per 8px band), deterministic and strict, both chains byte-exact; classic paths byte-identical (wrappers over the new _prev/_with_state internals), every staged-fixture identity test still green
- $7.2.6 UsePrevFrameMvs writer model - previous-frame-MV modeling closes the r418 milestone: InterFrameTreePlan::prev_frame_mvs carries the previous frame's $6.4.4 RefFrames/Mvs write-back (the _with_state assemblers' own product) into the $6.5.10 candidate scan via the shared FrameStateMvSource, making non-error-resilient SHOWN P-frame chains - and compound WITHOUT a hidden/intra predecessor - codeable; coherence validated at plan level (prev on ER header / prev+absent contradiction / mis-sized arrays rejected) and block level (use flag must match field presence); pinned by a NEARMV vector reachable ONLY through the prev-frame pass (same plan without the field is Unsupported), sample-identity vs an error-resilient NEWMV twin through the decoder's real $7.2.6 derivation, and a shown-chain compound frame whose every ZEROMV sample equals the independent Round2(LAST+ALTREF,1) average
- multi-tile frame writers - the tree assemblers accept any $6.2.13 tiling, mirroring the $6.4 decode_tiles() row-major walk (per-tile $9.2 coder brackets, f(32) tile_size prefixes on all but the last tile, above-context strips carrying across tiles with per-tile-row left resets incl. each tile's first row, $6.5 scans clamped to the tile's MiColStart/MiColEnd, AvailL = MiCol > MiColStart at column edges); pinned by reconstruction-identity across row splits at the staged fixture's 2col x 4row tiling, a degenerate empty-tile-row geometry, an all-inter tiled P-frame equal to its single-tile assembly with tiling-invariant per-MI write-back, a pinned reconstruction DIFFERENCE across a column split (intra AvailL vanishes exactly where the decoder's does), and the $6.4.1 tile-size walk consuming the frame exactly
- consume staged tiles-2col-4row-inter fixture - tile ROWS corpus-validated, 43/43 byte-exact with no code change (the $6.4 tile-row walk was already correct); presence-gated corpus-sweep wiring + round-434 flags pin (tile_rows_log2=2 AND tile_cols_log2=1 on both frames so a regenerated stream can't drop the row split); encoder-tooling-gap ledger closed (the black-box encoder clamps tile rows to 0 above one thread - threading interaction, not a missing feature); README ledger + stale tooling-gap notes refreshed
- README refresh for round 420 - encode-side $8.8 loop filter with per-frame level/sharpness election, filtered reference chains, A/B evidence (level 49, -7.6% GOP SSE at identical rate), lossy-filtered-gop corpus extension awaiting docs staging, oracle-carrying lossy-keyframe fuzz target, 1055+ unit tests
- encode_lossy_keyframe fuzz target - the first oracle-carrying harness: fuzz-derived (w, h, q, cycled content) through encode_vp9_lossy -> decode_vp9 with a stream-the-encoder-emitted-MUST-decode oracle (the self-encoded frame runs the full $8.1 chain incl. the elected $8.8 filter); 300-case deterministic smoke of the body passed locally across random partial-MI geometries
- loop_filter_sharpness election + the standalone lossy keyframe API elects - elect_filter_params runs a bounded two-stage search over both free $6.2.8 axes (64-level sweep at the header's sharpness, then the 0..=7 sharpness sweep at the winning level; the $8.8.4 Clip3(1, 9 - sharpness, lvl >> shift) limit trade moves only on a strict SSE win), both sequence encoders code the elected pair, and encode_keyframe_lossy_420 (public encode_vp9_lossy) closes out through the same election - a standalone keyframe's decoded output IS its filtered recon, so the election lifts display quality rate-free (72x56 partial-MI probe elects level 21, byte-exact through the black-box reference decoder); encode-side sharpness pinned decoder-mirror sample-exact at sharpness 0/3/7 + stage-2 never-regress pin
- filtered-GOP A/B pin + self-encoded lossy-filtered-gop fixture - the election's evidence test-pinned (keyframe elects level 49, GOP SSE 85340 -> 78850 at an identical 443 B total rate vs filtering forced off); build_lossy_filtered_gop_stream emits the corpus's first stream with encoder-elected non-zero loop_filter_levels (64x64 x4 via the public lossy sequence API, levels 19/27/19/24), byte-deterministic with staging generator + docs-gated staged-bytes identity + presence-gated corpus-sweep/flags-pin wiring; black-box reference decode verified byte-exact against the crate's own decode for the staged package plus a 76x52 partial-MI fixed-q GOP and a rate-controlled GOP
- encode-side $8.8 loop filter + per-frame filter-level election - assemblers gain _with_state variants returning the $6.4.4 Vp9FrameState per-MI arrays (the $8.8.2/$8.8.4 input, bit-identical to the decoder's); recon_filter replays $8.1 step 2 / $8.8 on the encoder's ReconState and elects loop_filter_level by full 0..=63 SSE-vs-source sweep (fixed-width $6.2.8 field - rate-free); the lossy sequence encoders (fixed-q + rc) close out every frame through the election and thread FILTERED reconstructions as references, mirroring the $8.10 post-filter store; decoder-mirror pinned sample-exact on all planes for intra + inter delta paths at multiple levels, election properties pinned, shared $7.2 delta-resolution helper deduplicated out of decode_frame
- README refresh for round 418 - 41/41 corpus, temporal-seg writer arm closed, sub-8x8 + compound election, $7.2 sign-bias finding
- $7.2 setup_past_independence sign-bias + PrevSegmentIds resets - error-resilient frames zero their effective ref_frame_sign_bias (compound uncodeable, no $6.5 sign flips) and clear the $6.4.14 predictor map; decoder applies both, writer derives all bias-dependent state from effective biases and rejects ER+compound / ER+temporal-seg; hidden inter frames + prev_frame_mvs_absent enable the valid non-ER compound shape; lossy-compound-elected rebuilt valid (black-box divergence found and closed), 41/41 corpus
- self-encoded lossy-sub8x8-elected stream + 40/40 corpus - the partition search's own sub-8x8 election corpus-pinned (VERT/HORZ/SPLIT leaves, per-cell NEWMV, exact prediction + skip); byte-exact vs black-box reference decode, flags pin, docs-gated staged-bytes identity
- sub-8x8 election in the lossy partition search - per-8x8-cell 4x4 quadrant probes elect VERT/HORZ/SPLIT below-8x8 leaves where divergent motion beats the single-vector SAD by a per-coded-vector margin; elected leaves code the $6.4.16 per-cell walk (snapped NEWMV/ZEROMV over LAST) with decoder-mirror sub-8x8 prediction + TX_4X4 skip/SSE-guard election; A/B pin: 56 B / SSE 0 vs 147 B / SSE 957 on opposing quadrant motion
- self-encoded temporal-seg-predicted stream + 39/39 corpus - hand-planned $6.4.12 seg_id_predicted/tree-escape placement over a visible SEG_LVL_REF_FRAME=GOLDEN band shifting across two temporal-update P-frames; byte-exact vs black-box reference decode, flags pin, docs-gated staged-bytes identity
- $6.4.12 temporal-predicted segment-id writer branch - seg_id_predicted against the $6.4.14 spatial-min predictor over PrevSegmentIds (flag=1 iff planned id == predictor, $6.4.7 tree escape otherwise) + trailing ctx-strip write-back via a shared SegPredContextState::write_back; tree assembler threads prev map + $7.4.1/$7.4.2 strip resets; round-trip pinned through the real $6.4.12 path-4 decode walk
- sub-8x8 chroma-geometry + HBD matrix closed - 4:2:2 and 4:4:0 sub-8x8 P-frames (two-cell chroma MV averaging, the third $8.5.2 averaging arm) and a 10-bit profile-2 sub-8x8 P-frame (HBD convolution clamps + WHT residual) all byte-exact end-to-end
- compound + 4:4:4 sub-8x8 verified end-to-end - lossless layout encoder gains an optional ALTREF second reference (compound [LAST,ALTREF] leaves incl. per-blockIdx sub-8x8 Round2 averaging, reference_mode=SELECT); byte-exact pins for compound sub-8x8 (two-list NEWMV + compound NEARESTMV seeds, ALTREF from a hidden intra-only slot), 4:4:4 sub-8x8 (per-cell chroma blockIdx vectors, no averaging), and switchable interp_filter on sub-8x8 blocks
- self-encoded render-size stream + 38/38 corpus - render_and_frame_size_different=1 on every frame across all three $6.2.3 render_size() arms (keyframe / intra-only / frame_size_with_refs); sub8x8-inter-mvs + render-size-128x72 join the whole-corpus byte-exact sweep with round-415 flags pin + docs-gated staged-bytes identity tests; README ledger: tile rows now the only fixture-less axis
- lossless layout P-frame encoder with sub-8x8 leaves - decoder-mirror per-blockIdx $8.5.2 prediction (averaged chroma vectors included) + $8.7.2 WHT residual over caller-chosen $6.4.3 layouts; self-encoded sub8x8-inter-mvs stream (six sub-8x8 nodes: 4x4/8x4/4x8, per-cell NEWMV/NEARESTMV/NEARMV/ZEROMV, integer + quarter-pel MVs) decodes byte-exact against its lossless targets
- sub-8x8 inter writer branches - $6.4.16 per-(idy,idx) walk through write_inter_block (per-cell inter_mode + $6.5.14 append_sub8x8_mvs predictor rewrite + $6.4.18 assign_mv, BlockMvs replication) and InterTreeLeaf::sub through the tree assembler; 4x4/4x8/8x4 leaves decode end-to-end incl. non-skip $6.4.21 residual at the 8x8 grid
- mark internal test-only public surface #[doc(hidden)]
- README - 36/36 corpus; sharpness + seg-feature axes closed by self-encoding; render-size remains the only fixture-less signalling axis
- self-encoded seg-features stream - SEG_LVL_SKIP/REF_FRAME/ALT_L + nonzero sharpness, byte-exact vs black-box reference decode (36/36 corpus); level-0 copy frame pins post-$8.8 reference stores
- segment-feature-aware inter block writer - SEG_LVL_SKIP / SEG_LVL_REF_FRAME no-bits arms + per-leaf segment_id through the inter tree assembler; round-trip pinned through the real $6.4.11 decode walk
- README refresh for round 412 - 35/35 corpus, 4:4:0 + 12-bit-inter matrix, temporal seg-map, intra-only writer branch, $8.8.2 onScreen fix; encoder-tooling-gap ledger (sharpness / render-size / seg ALT_L-SKIP-REF)
- whole-corpus sweep 35/35 byte-exact - round-412 corpus extension x9 (4:4:0, 12-bit inter x2, temporal seg-map prediction, 4 tile columns, BT.709 full-range, RGB inter, odd/partial geometry, intra-only) + per-fixture flags pin
- self-encoded odd-luma stream (59x37, chroma 30x19) - encoder pipelines round to even; lossless GOP with NEWMV motion decodes byte-exact at the odd geometry
- $6.2 intra-only + hidden-inter header-writer branches - keyframe assemblers accept intra-only headers (FrameIsIntra body identity); self-encoded intra-only stream (reset_frame_context=3, show-existing + inter over an intra-only slot) decodes byte-exact against source patterns
- $8.8.2 loop-filter views at the MI-aligned working extent - onScreen tests 8*MiCols/8*MiRows, not the visible crop; partial-MI 58x36 fixtures (420 odd-chroma + 444 regression stream) byte-exact
- README + module-doc refresh for round 409 (24/24 corpus, $8.4 adaptation section, scaled-ref + tile-rows status); drop stale dead_code allows in prob_adapt/mv
- corpus extension x3 - lossless-inter, tiles-2col-inter, hbd-backward-adaptation (24/24 byte-exact; flag-pinning test per class)
- $8.5.2.3 scaled-reference decode corpus-validated - self-encoded mid-GOP size-change stream, 21/21 byte-exact
- $6.1.2 refresh_probs backward adaptation wired - non-frame-parallel streams decode byte-exact (20/20 corpus)
- $9.3.4 non-coefficient counting - every counted element accumulated at its decode site
- uv_mode_probs joins the persisted FrameContext bank ($8.4.4 adapts it; $6.4.15 inter-frame intra blocks read the threaded copy)
- $9.3.4 coefficient counting - counts_token/counts_more_coefs collected per frame, more_coefs special case per errata #249 part 1
- clip $8.5.2 prediction stores at the working extent - frame-edge inter leaves on partial superblocks
- fuzz-found get_uv_tx_size out-of-range lookup on nonconformant chroma geometry
- corpus extension - 4:2:2 inter + intra-blocks-in-inter fixtures byte-exact
- whole-corpus sequence sweep - all 16 fixtures byte-exact through decode_vp9_sequence
- clear() over truncate(0) - CI clippy 1.97 manual_clear
- $8.1 step-2 loop-filter gate (level==0 disables $8.8 entirely) - full 16-fixture corpus byte-exact
- $7.2.10 segmentation-feature + $7.2.8 loop-filter-delta persistence across frames - segments-aq-mode 4/4 byte-exact
- NEARESTMV/NEARMV mode mapping - predictor-equal vectors drop the mv-diff syntax
- README + CHANGELOG - round 391 encoder depth (inter trees, tx selection, sub-pel, multi-ref, compound) + decoder OOM guard
- compound inter encode - [LAST, ALTREF] Round2 average election on the tree encoder
- multi-reference inter encode - per-leaf LAST/GOLDEN election with a long-term keyframe reference
- sub-pel motion search - decoder-mirror $8.5.2 refinement to quarter/eighth-pel
- content-adaptive inter partition tree + per-leaf inter tx selection + RD skip guard
- $6.4.3 inter tree assembler - arbitrary partitions + per-block inter tx selection
- bound decoder allocations by the VP9 level-table max luma picture size
- add CI / crates.io / docs.rs / MIT-license badges
- README + CHANGELOG — round 387 encoder depth (large transforms, tree planner, TX_8X8 P-frames, skip election, rate control)
- per-block skip election for P-frames — static blocks code no residual
- rate control — per-frame quantizer bisection to a byte budget
- lossy P-frames upgrade to inferred TX_8X8 (Allow8x8)
- >4x4 transform lossy encoder + content-adaptive superblock-tree planner
- tree-plan keyframe assembler — arbitrary partitions + per-leaf transform sizes
- general §6.4.3 partition-tree writer — caller-chosen NONE/HORZ/VERT/SPLIT layouts
- forward ADST8/ADST16 + TX_32X32 dqDenom quantization — full-size forward-transform coverage
- README — pixel-accurate encoder (lossless all-profile keyframes, motion-compensated lossless/lossy sequences)
- lossy video sequence encoder — motion-compensated quantized-DCT chain with a bit-exact decoder mirror
- NEWMV integer motion search for the lossless P-frame encoder
- forward ADST4 + intra mode selection for the lossy encoder
- lossy keyframe encoder — quantized DCT residual with a bit-exact decoder mirror
- lossless inter (P-frame) sequence encoder — byte-exact through decode_vp9_sequence
- lossless keyframe encoding across all four VP9 profiles
- lossless pixel-exact keyframe encoder — encode_vp9 round-trips byte-exact
- forward transforms — exact lossless WHT inverse + scaled forward DCT
- README — inter-encode writers + inter frame assembler
- §6.2/§6.3/§6.4 inter frame assembler + public encode_vp9_pframe_sequence
- §6.3 inter compressed-header writer (default-probability path)
- §6.2 inter uncompressed-header writer (non-intra-only branch)
- §6.4.11/§6.4.16 inter block writer (mode-info + MV + residual)
- §6.4.13/§6.4.16/§6.4.17 inter mode-info writers
- §6.4.18-§6.4.20 motion-vector writers (first inter-frame encode leaf)
- AC-residual end-to-end reconstruction pin through the frame assembler
- README + CHANGELOG — document the complete keyframe encoder path
- encoder public-surface integration suite + encode-path fuzz target
- per-block segment_id in the keyframe assembler + segmentation / 10-bit / determinism pins
- non-skip residual end-to-end reconstruction pins + flat-fill invariant
- top-level keyframe frame assembly — encode_vp9 produces a decodable keyframe
- §6.4.4 keyframe intra block writer — inverse of decode_block_intra, round-trip validated
- §6.4.21 residual encode driver — inverse of BlockDecoder::residual, round-trip validated
- §6.4.24 block-level write_tokens driver — inverse of tokens() decode, round-trip validated
- §6.4.3 partition-syntax writer — inverse of decode_partition, round-trip validated
- README — document the encoder-bootstrap bitstream-writer primitives
- §6.4.6 keyframe intra mode-info writer — inverse of the decoders, round-trip validated
- §6.4.24 coefficient-token writer — inverse of the token decode, round-trip validated
- §6.3 compressed-header writer (intra, default-prob path) — round-trip validated
- §6.2 uncompressed-header writer — inverse of the parser, round-trip validated
- §9.2 Boolean range encoder — inverse of the decoder, round-trip validated
- scrub hyphenated algorithm-name forms from test/CHANGELOG prose
- README — document the independent-oracle validation layer + decode-path fuzz/robustness
- pin §8.5.1 intra whole-mode invariants — flat-preservation + Clip1 bound
- pin §9.3.2 coefficient-context neighbour causality across every scan
- complete §8.7 transform cross-validation — inverse DCT closed form + WHT matrix/involution
- independent closed-form cross-validation of the §8.7 inverse DCT + ADST4
- full-pipeline panic-surface fuzz target + decode_robustness pins
- README — §8.4.4 adapt_noncoef_probs completes the §8.4 primitive set
- §8.4.4 adapt_noncoef_probs — non-coefficient probability adaption
- pin corpus divergence profiles as regression boundaries + correct frame-2 diagnosis
- pin segments-aq frames 0-1 byte-exact + sharpen divergence diagnosis
- README + CHANGELOG — round 357 frame-context entropy threading
- §6.4.14/§8.1 PrevSegmentIds only refreshes after an update_map frame
- §6.1.2/§7.2 frame-context entropy threading — load_probs/save_probs across frames
- pin show-existing-frame corpus keyframe byte-exact + document inter divergence
- §8.9 show_existing_frame sources per-slot FrameStore (incl. hidden ARF slots)
- README + CHANGELOG — round 349 §8.4 backward-adaptation primitives + more_coefs docs gap
- §8.4.3 adapt_coef_probs — coefficient backward-adaptation transform
- §8.4.1/§8.4.2 merge_prob + merge_probs backward-adaptation primitives
- validate scaled-reference + compound inter prediction vs spec re-derivation
- Annex B superframe split + four-frame inter sequence byte-exact
- end-to-end inter-frame (P-frame) decode — §6.4.4 decode_block inter arm + decode_vp9_sequence multi-frame driver
- neutralise filter-numbering test comment wording
- README + CHANGELOG — round 339 §8.5.2 MC driver + §8.10 ref buffers + §6.2 inter header + adapters
- inter-frame decode adapters — Vp9FrameState MvCandidateSource + RefBuffers RefPlanes builder
- §6.2 inter (non-intra-only) uncompressed-header parse — frame_size_with_refs + interp filter
- §8.10 reference frame-buffer state — FrameStore slots + update + refIdx resolution
- §8.5.2 inter prediction driver — chain select/clamp/scale/predict into CurrFrame
- §8.5.2.1/§8.5.2.2/§8.5.2.3 motion-vector selection, clamping, and scaling
- §8.5.2.4 block inter prediction — two-pass 8-tap sub-pixel convolution
- §6.4.11 inter_frame_mode_info( ) driver — per-block prelude + §6.4.5 dispatch
- §6.4.16 inter_block_mode_info driver — full per-block inter mode-info decode
- §6.4.17 read_ref_frames( ) driver — single + compound + segment ref-frame resolution
- §9.3.2 comp_mode probability-context derivation
- refresh to current status, drop per-round changelog cruft

### Changed

- **chained-as-default — the §7.2.6 chain framing IS the public GOP path** — [`encode_vp9_lossless_sequence`] and [`encode_vp9_lossy_sequence`] now emit shown **non-error-resilient** P-frames (the decode-side §7.2.6 `UsePrevFrameMvs` derivation is 1; the encoder threads each frame's §6.4.4 motion field into the next frame's §6.5 predictor derivations and block writer), completing the r434 "later milestone": temporally persistent motion codes `NEARESTMV` / `NEARMV` instead of §6.4.20 mv-diff bits, the `[ LAST, ALTREF ]` compound election is live inside the ordinary shown GOP (a non-error-resilient frame keeps its coded sign biases per §7.2 `setup_past_independence( )`), and the round-441 keyframe skip + §6.2.8 loop-filter delta elections run on the default lossy path with the §7.2.8 persistent baseline threaded writer-side. The classic framing becomes the **explicit opt-out** — new [`encode_vp9_lossless_sequence_error_resilient`] / [`encode_vp9_lossy_sequence_error_resilient`] entries code §6.2 `error_resilient_mode = 1` P-frames with the exact **frozen** pre-445 bytes (the staged self-encoded corpus fixtures `odd-dims-59x37` / `lossy-filtered-gop` pin them byte-for-byte; their builders now name the opt-out). The `_chained` entries remain as byte-identical aliases of the defaults. Byte-stability contract documented on every entry: the default entries' bytes changed in this round (framing + elections), the opt-outs are frozen, and every entry point stays byte-deterministic within a crate version — pinned by chain-framing header sweeps on both defaults, alias byte-identity, opt-out `error_resilient_mode = 1` sweeps, lossless byte-exactness on both framings, and the unchanged staged-fixture identity tests (round 445)

### Added

- **4:4:0 framework labels — the core 0.1.35 consumer wave** — the framework now carries the 4:4:0 planar family (`PixelFormat::Yuv440P` / `Yuv440P10Le` / `Yuv440P12Le`, `ssx = 0, ssy = 1`, full-width / half-height chroma), so the last §7.2.2 geometry gets its registry label: the registry Decoder decodes 4:4:0 streams (the staged `profile-1-yuv440-8bit-inter` and the self-encoded `lossy-440-gop` / `lossy-hbd12-440-gop` packages) instead of surfacing `Unsupported` at the frame-conversion boundary, with chroma planes at the label's own `plane_dimensions( )` extent (§8.10 `ceil` on the row axis); new public [`pixel_format_for_triple`]`( BitDepth, ssx, ssy )` is total over the twelve §7.2.2 triples and `Vp9Decoder::pixel_format( )` reports the label of the most recently decoded frame (cleared by `reset( )`); the registry Encoder accepts `Yuv440P*` input (profile 1 at 8-bit, profile 3 at 10/12-bit) over the already subsampling-generic lossy GOP path, byte-identical to [`encode_vp9_lossy_sequence_440`] / [`encode_vp9_lossy_sequence_hbd_440`]; `register( )` advertises the full 4 x 3 = 12 format matrix. Pinned by the corpus sweep (every fixture's decoder label + per-plane extents against the framework geometry, 4:4:0 no longer special-cased) and a new odd-height 8/10/12-bit 4:4:0 registry round-trip (packet bytes, header `ssx`/`ssy`, decoded labels, sample-exact planes) (round 452)

- **intra-only frame emission — `Vp9GopConfig::intra_only_altref`** — each alt-ref group's hidden frame can be coded as a §6.2 **intra-only** frame (`frame_type = NON_KEY`, `show_frame = 0`, `intra_only = 1`, `reset_frame_context = 3`, single-slot `refresh_frame_flags`): a mid-GOP refresh point that cuts the prediction chain without a keyframe in the display stream, surfacing later through its `show_existing_frame` packet. The keyframe planner + tree encoder now accept the §6.2 `FrameIsIntra` predicate (key frame, or hidden intra-only — `mode_info( )` dispatches on `FrameIsIntra`, so both code the identical §6.3/§6.4 intra body; the profile-0 default color config the §6.2 intra-only branch installs matches the 8-bit 4:2:0 entry), and the encoder mirrors the decoder's §7.2 `setup_past_independence( )` on it: the §7.2.8 delta baseline and §7.2.10 feature table restart (the frame codes `update_data = 1`), `PrevSegmentIds` is replaced by the frame's own `update_map = 1` walk (§8.1 step 3 — the next frame predicts its map from it), and the next decoded frame codes with the prev field absent (§7.2.6 (c) and (e)). Pinned by a decoder-mirror with the full segmentation emission stacked on, header pins (re-parsed from the packet bytes: the §6.2 intra-only branch), the config-entry contract, and the stream locally **byte-exact** through the black-box reference decoder; a fifth fixture package `intra-only-emitted-gop` joins the round's staging generator (round 452)

- **five round-452 fixture packages built + wired (staging pending)** — new self-encoded stream classes with no corpus precedent, each generated deterministically through the PUBLIC entries (`stage_round_452_fixtures_when_requested` under `OXIDEAV_VP9_STAGE_DIR`, emitting `input.ivf` + the crate's own decode as `expected.yuv`) and locally verified **byte-exact** against the black-box reference decoder (timestamp passthrough; auto-scale disabled for the resized stream): `altref-pyramid-gop` (the corpus's first ENCODER-MINTED hidden alt-ref + `show_existing_frame` stream — three-slot `[ LAST, GOLDEN, ALTREF ]` election, 3 356 B / 36 864 B), `seg-emitted-full-gop` (all four `SEG_LVL_*` features emitted with fitted tree/pred probabilities, temporal map updates and the persistent feature table — the corpus's `seg-features-*` fixtures are black-box encodes; this one pins the writer side, 1 802 B / 27 648 B), `tiles-2col-encoded-gop` (the first SELF-ENCODED multi-tile-column stream — the §9.2.4 tile-parallel decoder's consumer-side twin, 6 768 B / 73 728 B), `intra-only-emitted-gop` (the first encoder-minted hidden intra-only frames — mid-GOP refresh points shown through `show_existing_frame` — with the full segmentation emission), and `resized-encoded-gop` (the first PIXEL-ACCURATE self-encoded §8.5.2.3 stream — the r409 `scaled-reference` fixture is all-skip ZEROMV, 9 485 B / 50 688 B). In-crate wiring complete: builder byte-determinism pins, the staging generator, and docs-gated staged-bytes + `expected.yuv` identity tests that activate once the packages land in `docs/video/vp9/fixtures/` (round 452)

- **`encode_gop_structures` fuzz target — the ninth harness, over the round-452 write paths** — fuzz-derived [`Vp9GopConfig`] axes (alt-ref pyramid interval 1..=4, all four §6.2.11 segmentation modes, tile rows) over fuzz-derived translating content, plus a §8.5.2.3 resized-sequence arm alternating between a geometry and its half-size twin inside the §5 ratio bounds. The oracle is stream-the-encoder-emitted-MUST-decode, strengthened structurally: the decode must return **exactly the source frame count of shown frames** (hidden alt-refs and `show_existing_frame` packets netting out through the display order) at the declared per-frame sizes — any error or count/size mismatch is an encoder-side §7.2.6 / §6.2.11 / §6.4.14 modeling bug by construction. Fuzz workflow roster comment refreshed (round 452)

- **scaled-reference encode — the encoder mints §8.5.2.3 mid-stream coded-size changes, pixel-accurately** — new [`encode_vp9_lossy_sequence_resized`] (8-bit 4:2:0) codes a keyframe at `sizes[ 0 ]` and one shown P-frame per later frame at its own §6.2.5 explicit size, predicted from the previous frame's §8.10 store through the **§8.5.2.3 scaled sampler** whenever consecutive sizes differ (`xScale = (RefFrameWidth << 14) / FrameWidth`; the underlying `predict_inter_leaf2` is generalised over distinct reference dimensions, equal sizes reducing to the unscaled path bit-for-bit). The partition tree is planned against a *virtual reference* (the whole-frame ZEROMV scaled prediction at the current size); each leaf then elects ZEROMV vs. a log-diamond NEWMV descent (eighth-pel steps 64..1) with **every candidate scored by the decoder's own §8.5.2.3 prediction**, the winner parity-snapped against the §6.5.12 `BestMv` and §6.4.16-mapped to NEARESTMV / NEARMV, followed by the ordinary per-leaf transform-size + skip elections — so the decoder-mirror stays exact across the resize. §7.2.6 (b) modeling (`UsePrevFrameMvs = 0` across a size change) and the §5 scaling bounds (at most 2x downscale / 16x upscale per axis, rejected otherwise) are enforced. This closes the r409 tooling-gap asymmetry: the `scaled-reference` corpus fixture was all-skip ZEROMV self-encoded because no black-box encoder CLI mints such streams — the crate's own encoder now codes *pixel-accurate* residuals over scaled prediction. Pinned by a 4-frame 2x-down / 4/3-up / back-to-full GOP decoding at its declared sizes within the quantizer's PSNR regime, byte-determinism, contract + rejection tests, and the dumped stream locally **byte-exact** through the black-box reference decoder (round 452)

- **tile columns + tile rows on the write side — the encoder now mints the streams the §9.2.4 tile-parallel decoder consumes** — [`Vp9GopConfig`] gains `tile_cols_log2` / `tile_rows_log2` (validated against the §6.2.14 min/max derivation at write time; two columns need a frame ≥ 8 SB64s wide), and the whole structured-GOP pipeline is tile-correct on the encoder side: intra prediction derives §6.4.4 `AvailL = MiCol > MiColStart` (the reconstruction-plane predictor severs the left neighbour at a tile's left edge exactly as the decoder's per-tile walk does; `AvailU` stays frame-level — the above strips carry across tile rows), and every §6.5 candidate scan in the P-frame planner — the leaf evaluation, the compound second-list re-snap, the NEARESTMV / NEARMV mode mapping, and the sub-8x8 cell walk — clamps its column window to the leaf's tile extents via the §6.4 `get_tile_offset( )` bracket (identity on single-tile frames, so every existing entry is byte-unchanged). The tile assemblers (per-tile §9.2 coder brackets, 4-byte size prefixes, per-superblock-row left-context resets including the tile's first row) were already in place from the r448 decode work. Pinned by decoder-mirrors on a 2-tile-column 512-px GOP (alt-ref pyramid framing) and a 2-tile-row GOP stacked with the full segmentation emission, a §9.2.4 **tile-parallel parity** sweep (the self-encoded multi-column stream decodes byte-identically through `decode_vp9_sequence_with` at a 4-thread budget), config-entry contract + rejection tests (narrow frames cannot code 2 columns; `tile_rows_log2 > 2` uncodeable), and both streams locally **byte-exact** through the black-box reference decoder (round 452)

- **§6.2.11 segmentation feature emission — all four `SEG_LVL_*` features on the write side, config-driven** — new [`Vp9GopConfig`] (`#[non_exhaustive]`, `Vp9GopConfig::new( base_q_idx )`) + [`encode_vp9_lossy_sequence_with`] generalise the GOP encoder over `altref_interval` and a [`Vp9Segmentation`] mode: `AdaptiveQuant` (four activity-class segments by per-leaf luma mean-absolute-deviation buckets carrying `SEG_LVL_ALT_Q` deltas `[-16, -6, +4, +12]` around `base_q_idx` — the whole quantization/reconstruction pipeline is per-segment-qindex aware down to `get_dc_quant` / `get_ac_quant` at every trial and replay site — plus a per-frame per-segment `SEG_LVL_ALT_L` election swept by full-§8.8-filter SSE, a moved table paid only on a strict win), `StaticSkip` (leaves whose co-located `LAST` prediction's quantized residual is all zero code on the `SEG_LVL_SKIP` + `SEG_LVL_REF_FRAME = LAST_FRAME` segment — §6.4.8 / §6.4.13 / §6.4.16 / §6.4.17 then read no skip / is_inter / ref_frame / inter_mode syntax and §6.4.21 no residual; the §6.4.16 `MiSize >= BLOCK_8X8` conformance bound is honored by keeping sub-8x8 leaves off the segment), and `Full` (both). Every decoded frame codes `segmentation_update_map = 1` with **per-frame fitted `segmentation_tree_probs`** (a probe encode collects the §9.3 `segment_tree` node counts, the final encode carries the fit; a never-visited node codes the 1-bit `read_prob( )` escape), `segmentation_temporal_update = 1` whenever a §6.4.14 `PrevSegmentIds` predictor exists (fitted `segmentation_pred_prob`), and the **§7.2.10 persistent feature table**: `update_data = 1` only when the elected table moved off the decoder's persisted copy. Keyframes assign activity classes over the planner tree (`intra_segment_id( )` tree writes); P-frames elect per leaf inside the planner. NOTE: §6.2.9 derives `Lossless` from the frame's `base_q_idx` alone — per-segment lossless does not exist in VP9 (a `SEG_LVL_ALT_Q` segment reaching qindex 0 still runs §8.6.2 at `dc_q( 0 )` / `ac_q( 0 )`), documented on the entry. Pinned by decoder-mirrors over all three modes on the pyramid GOP (every shown frame equal to the encoder reconstruction), header pins (temporal_update off exactly on the keyframe, the coded feature table, ≥ 1 update-free frame), coded-map pins (the static half of a half-static scene lands on the skip segment, the keyframe spans ≥ 2 activity classes, tree probs fitted), the config-entry public contract (thin-wrapper byte-identity with the altref entry, PSNR floor, determinism, rejections), and three black-box dump cases (`seg-full` / `seg-aq` / `seg-skip`) — locally **byte-exact** through the black-box reference decoder (round 452)

### Fixed

- **§6.4.14 `PrevSegmentIds` wrongly invalidated by a hidden predecessor** — the decoder gated the whole previous-frame snapshot (motion field AND segment map) on `show_frame == 1` of the preceding decoded frame. That gate belongs to §7.2.6 `UsePrevFrameMvs` condition (c) ONLY: §8.1 step 3 refreshes `PrevSegmentIds` after **every** decoded `update_map = 1` frame, shown or hidden, and no §7.2.6 condition touches it — so a frame coding `segmentation_temporal_update = 1` right after a hidden alt-ref mis-predicted every `seg_id_predicted` block from a zero map (small per-sample drift through the wrong per-segment qindex / filter level). Found by the round-452 segmentation encoder's decoder-mirror (the first in-corpus stream combining a hidden alt-ref with a temporal map update on its successor); the fix threads `prev_shown` through the snapshot for the §7.2.6 gate alone, and the `seg-full` dump case pins the corrected behaviour **byte-exact** against the black-box reference decoder (round 452)

- **alt-ref pyramid GOP encoder — hidden alt-refs, `show_existing_frame`, and a three-slot `[ LAST, GOLDEN, ALTREF ]` election** — new [`encode_vp9_lossy_sequence_altref`] (8-bit 4:2:0) codes groups of `altref_interval` display frames as a **hidden** (`show_frame = 0`) P-frame of the group's last source frame into a free §8.10 slot, then the group's earlier frames as shown P-frames over `ref_frame_idx = [ last, golden, alt ]` with `ref_frame_sign_bias = [ 0, 0, 1 ]` (the alt-ref genuinely lies in the future, so the §6.5 candidate sign flip is the semantically right one), and closes the group with a one-byte §6.2 `show_existing_frame` packet displaying the alt-ref at its display position — the alt-ref slot then becomes `LAST` and the freed slot hosts the next group's alt-ref (the keyframe stays the long-term `GOLDEN`). The inter tree encoder gains a distinct third reference (`encode_pframe_lossy_tree_motion_with_state_alt`): every leaf now sweeps single `LAST` / `GOLDEN` / `ALTREF` (ZEROMV + searched, sub-pel-refined NEWMV each) and the `[ LAST, ALTREF ]` §6.3.18 compound pair over the *true* alt-ref planes; the two-slot entries pass `None` and stay byte-identical (the GOLDEN alias). §7.2.6 modeling follows `compute_image_size( )` to the letter: the frame decoded right after a hidden alt-ref codes with the prev field absent (`show_frame == 0` at the previous invocation), and a `show_existing_frame` packet invokes neither `compute_image_size( )` nor the §6.1.2 `PrevMvs` save, so the frame after it still models the last *decoded* frame's motion field; the §7.2.8 delta baseline persists across every packet. Pinned by a decoder-mirror (every shown frame — the alt-ref surfacing at its `show_existing_frame` position included — equal sample-for-sample to the encoder's reconstruction, packet structure `1 + (interval + 1)·groups`, slot rotation visible in the `frame_to_show_map_idx` bytes, byte-determinism), a cross-fade A/B (the midpoint frame codes under half the two-slot chain's bytes through the future-alt-ref compound), the interval-1 degenerate chain, a rejection sweep, and a `gop-altref-pyramid` black-box dump case — locally **byte-exact** through the black-box reference decoder (run with timestamp passthrough, since the hidden packets carry IVF timestamps) (round 452)

- **tile-parallel decode — the §9.2.4 multi-coder path lands, ExecutionContext-governed** — multi-tile-column frames now decode their tile columns concurrently (one live §9.2 boolean decoder per in-flight tile), with `oxideav_core::ExecutionContext` as the **single threading authority**: the fan-out is `exec.effective_workers( tileCols )`, serial until told otherwise, and never a host query. New surfaces: `decode_vp9_sequence_with( frames, &ExecutionContext )`, `Vp9SequenceDecoder::set_execution_context`, and the framework `Vp9Decoder` now honors the registry `set_execution_context` hook (budget preserved across `reset( )` — stream configuration, not stream state). The parallel unit is the tile **column** (every tile row of it, in row order): VP9 makes columns independent — §6.4.4 `AvailL = MiCol > MiColStart` severs left neighbours, tile boundaries are SB64-aligned so no block or §8.5.1 edge read crosses `MiColEnd` (the `aboveRow` reads are bounded by the block's own width; the above-right extension fires only for a `txSz == 0` block `notOnRight` of its parent — still inside the block), and the §6.5 candidate scans are clamped to the tile window — so each worker decodes its columns into private full-extent state and the disjoint MI column ranges merge back deterministically (`Plane::copy_cols_from`, `Vp9FrameState::merge_cols_from`, `FrameCounts::merge_from` — the latter two exhaustively destructured so a future field cannot silently drop out of the merge; count summation is order-independent, so the §6.1.2 `refresh_probs( )` input is exact). **Output is byte-identical to the serial walk at every budget**: pinned by serial-vs-parallel sample AND §9.3.4-count-bank equality on live-residual 2col x 4row keyframes, an uneven-column split, and NEWMV-straddling tiled P-frames at 2/3/8-thread budgets; a whole-corpus sweep (every staged fixture, 4-thread budget) byte-identical to serial; and a registry-surface parity + reset-persistence sweep over the four staged tile fixtures. Both decode paths run tiles through the one extracted §6.4.2 `decode_one_tile( )` body, so serial and parallel cannot drift. Measured (release, 1080p 4-tile-column black-box-encoded streams, 60 frames): **3.10x at 4 threads / 1.78x at 2 threads** on a coefficient-heavy stream (serial 7.86 s → 2.53 s), 1.31x at 4 threads on an easy low-rate stream (the §8.8 loop-filter + merge stay serial); env-gated `OXIDEAV_VP9_TILE_BENCH_IVF` timing probe committed alongside (asserts byte-identity, prints wall times, never gates CI) (round 448)

- **`decode_tile_parallel_diff` fuzz target — a total differential oracle over the tile-parallel path** — the eighth harness decodes every fuzz input twice (the serial walk vs the 4-thread tile-column fan-out) and requires the identical outcome: byte-for-byte equal shown frames on `Ok`, the equal error value on `Err` (tiles are independent, so the failing-tile set is schedule-invariant and the parallel path surfaces the lowest raster-order tile error — the one serial hits first), and any `Ok`/`Err` split is a bug by construction. Single-column inputs pin the `effective_workers` clamp; `tile_cols_log2 > 0` headers drive the real fan-out, worker chunking, and column-range merges. Fuzz workflow roster comment refreshed (round 448)

- **4:4:0 public encoder entries — the last §7.2.2 geometry gets its dedicated surface** — new `encode_vp9_lossy_440` / `encode_vp9_lossy_hbd_440` keyframe entries and `encode_vp9_lossy_sequence_440` / `encode_vp9_lossy_sequence_hbd_440` chain-framed sequence entries cover `subsampling_x = 0, subsampling_y = 1` (profile 1 at 8-bit, profile 3 at 10/12-bit; chroma planes `w × ceil(h/2)`; decode-side the §8.5.2.1 chroma MV derivation rounds only the row component) — thin public wrappers over the already-subsampling-generic lossy pipeline the 9-format decoder-mirror sweep has pinned since round 441. Public-contract tests (coded-format assertions, §8.10 chroma extents, distortion regime, byte-determinism, no-drift 4-frame GOPs at 8-bit and 12-bit, rejection sweep) plus four new black-box dump cases (`lossy-440-8bit` / `lossy-440-12bit` keyframes, `gop-440-8bit` / `gop-440-12bit` chained GOPs); local sweep: all **23 dumped streams decode byte-exact** through the black-box reference decoder against the crate's own decode. The framework registry surface is unchanged — the framework has no 4:4:0 pixel-format label, so the registry Decoder keeps its explicit `Unsupported` at the frame-conversion boundary and the direct crate APIs carry the geometry (round 448)

- **two round-448 fixture packages built + wired (staging pending)** — `lossy-440-gop` (the corpus's first **self-encoded** 4:4:0 stream, profile 1, via `encode_vp9_lossy_sequence_440`; the staged `profile-1-yuv440-8bit-inter` covers the geometry as a black-box encode, this one pins the writer side) and `lossy-hbd12-440-gop` (profile 3, 12-bit, 4:4:0 — the only §7.2 format-matrix corner with **no corpus fixture at all**, via `encode_vp9_lossy_sequence_hbd_440`). Each package is generated deterministically (`stage_round_448_fixtures_when_requested` under `OXIDEAV_VP9_STAGE_DIR`, emitting `input.ivf` plus the crate decode in the corpus `expected.yuv` packing) and locally verified **byte-exact** between the black-box reference decode and this crate's own `decode_vp9_sequence` output (24 576 / 49 152 bytes); in-crate wiring is complete — shape pins (profile / bit-depth / `ssx = 0, ssy = 1` geometry, chain-framing headers, byte-determinism), docs-gated staged-bytes identity, presence-gated whole-corpus sweep entries, and a header-flags pin so a regenerated stream cannot silently flip to a covered geometry (round 448)

- **black-box dump harness extended to the round-445 default paths** — `dump_matrix_streams_for_black_box_validation` (env-gated, `OXIDEAV_VP9_MATRIX_DUMP_DIR`) gains three GOP cases riding the promoted defaults: `gop-420-8bit-default` (the default lossy sequence entry), `gop-lossless-chained` (the default lossless entry — chain framing + skip-elected keyframe), and `gop-420-8bit-rc` (the rate-controlled chain). Local run: all **19 dumped streams — the 10 keyframe matrix formats and all 9 GOP classes — decode byte-exact** through the black-box reference decoder against the crate's own decode (round 445)

- **three round-445 fixture packages built + wired (staging pending)** — new black-box byte-exact stream classes on the round-445 **chained defaults**: `lossless-chained-gop` (the corpus's first chain-framed LOSSLESS stream — §7.2.6 `UsePrevFrameMvs == 1` over §8.7.2 WHT residuals — and its first skip-elected lossless keyframe, through the now-default `encode_vp9_lossless_sequence`), `lossy-hbd12-422-gop` (the first self-encoded stream at the §7.2 matrix's deepest corner: profile 3, 12-bit, 4:2:2 — CAT6 18-bit tokens through the whole chained lossy pipeline, via `encode_vp9_lossy_sequence_hbd_422`), and `lossy-rc-gop` (the corpus's first RATE-CONTROLLED stream: per-frame bisected `base_q_idx` on the chain framing with the budget-guarded §6.2.8 delta election, via `encode_vp9_lossy_sequence_rc`). Each package is generated deterministically (`stage_round_445_fixtures_when_requested` under `OXIDEAV_VP9_STAGE_DIR`, which also emits the crate's own decode in the corpus `expected.yuv` packing for the black-box compare) and locally verified **byte-exact** between the black-box reference decode and this crate's own `decode_vp9_sequence` output (18 432 / 49 152 / 18 432 bytes); in-crate wiring is complete — shape pins (chain-framing headers, skip-elected-keyframe strict-shrink, profile-3/12-bit/4:2:2 geometry, per-frame budget), docs-gated staged-bytes identity, and presence-gated whole-corpus sweep entries (round 445)

- **round-441 elections threaded onto every chained public entry** — (1) the **rate-controlled chain** (`encode_vp9_lossy_sequence_rc`) rides the §7.2.6 chain framing: shown non-error-resilient P-frames with prev-frame-MV modeling (the keyframe's §6.4.4 field — invariant under the filter re-encodes — seeds the model), the keyframe **skip election** inside the per-frame quantizer bisection (a strict rate win, so the fitted `base_q_idx` can only improve), and the §6.2.8 **delta election under the byte budget**: a moved slot codes 1 + 7 §6.2.8 update bits, so `finish_frame_with_filter_deltas_budget` falls back to the update-free frame whenever the update would overflow `target_bytes_per_frame` — the fallback still filters on the §7.2.8 *persisted* baseline exactly as the decoder's fold does, its length equals the fitted trial's (level / sharpness are fixed-width), and the persistent state stays unmoved on both sides; budget compliance, chain-framing headers, delta-content dual-budget sweeps, and determinism are pinned. (2) The **chain-framed default lossless keyframe elects skip** (`encode_keyframe_lossless_elect_skip`, two-pass token-cache mirror of the lossy election): an MI whose entire WHT residual is zero — exact §8.5.1 DC prediction, since the §8.7.2 WHT is invertible — codes `skip = 1`, a strict rate win at an identical (still byte-exact lossless) reconstruction; flat content shrinks strictly, noise is byte-identical (no elected MI), and the classic `_error_resilient` entries keep the non-electing writer (staged-fixture-pinned bytes). LF-delta election stays inapplicable at lossless (level 0 codes no filtering per §8.1 step 2) (round 445)

- **framework registry integration — `Vp9Decoder` / `Vp9Encoder` + `make_decoder` / `make_encoder`, the Encoder riding the chained default** — `register( )` stops being a no-op: it installs a decode+encode video registration (lossy + lossless, the nine §7.2 format-matrix pixel formats, wire-tag claims `VP90` / `VP09` / `V_VP9`) with both factory paths of the dual-API convention. The **registry Encoder rides the §7.2.6 chain-framed default GOP path** — packet bytes are **byte-identical** to the matching public batch entry (`encode_vp9_lossy_sequence`, `encode_vp9_lossless_sequence` under `lossless=true`, and the format-matrix variants at 10/12-bit and 4:4:4/4:2:2), pinned per-packet; options are `q` (§7.2.9 `base_q_idx`, 1..=255) and `lossless` (8-bit 4:2:0 only — the lossless GOP path's format). The **registry Decoder is the new incremental [`Vp9SequenceDecoder`]** — the stateful form of `decode_vp9_sequence` (that function is now a thin loop over it, corpus-pinned byte-exact), owning the §8.10 reference buffers + `FrameStore[ ]`, the §6.5 previous-frame motion field, the §6.1.2/§7.2 `FrameContext[ 4 ]` banks with §8.4 backward adaptation, and the §7.2.8/§7.2.10 persistent header state — with the §B.2 Annex B split applied per packet, so superframes / hidden alt-refs / `show_existing_frame` work at the registry surface. Under the hood the chained sequence engines gained stateful push forms (`LossyGopEncoder` / `LosslessGopEncoder420`; the batch entries are thin loops over them, byte-for-byte — the staged r441 fixture identity tests pin it). Registry coverage: a whole-corpus packet-by-packet sweep equal to the batch decode (43 fixtures; the 4:4:0 stream asserts its no-framework-label `Unsupported` surface), byte-identity pins against four batch entries, stride-padded input de-striding, `reset( )` fresh-stream semantics, pts passthrough + keyframe packet flags, and a parameter-rejection sweep (round 445)

- **`encode_lossy_matrix` fuzz target + in-CI deterministic smoke** — a third oracle-carrying harness: fuzz-derived `(format selector over all seven public-entry §7.2 formats, width/height 1..=64, base_q_idx, cycled bit-depth-masked content)` runs the matching matrix keyframe entry (`encode_vp9_lossy` / `_444` / `_422` / `_hbd` / `_hbd_422`) with the stream-the-encoder-emitted-MUST-decode oracle (the self-encoded frame runs the full §8.1 chain — elected §8.8 filter and skip-elected leaves included). A 28-case LCG-deterministic smoke of the same body (`lossy_matrix_smoke_over_random_geometries`) runs in standard CI; the fuzz workflow's stale two-target header comment is refreshed to the actual seven-target roster (round 441)

- **three round-441 fixture packages built + wired (staging pending)** — `lossy-444-gop` (the corpus's first self-encoded non-4:2:0 lossy stream: profile 1 chain framing through the public `encode_vp9_lossy_sequence_444`), `lossy-hbd10-gop` (the first self-encoded high-bit-depth lossy stream: profile 2, 10-bit), and `lossy-lf-deltas-gop` (the first stream with a `loop_filter_delta_update = 1` frame: the delta election codes a mode-delta update mid-GOP and a later frame filters on the §7.2.8 **persisted** value update-free, so the byte-exact sweep pins the persistence fold against the reference decode). Each package (input.ivf + black-box expected.yuv) is generated deterministically (`stage_round_441_fixtures_when_requested` under `OXIDEAV_VP9_STAGE_DIR`) and locally verified **byte-exact** between the reference decoder and this crate's own decode; in-crate wiring is complete — behavior pins (profile/format headers, update-then-persist shape), docs-gated staged-bytes identity, presence-gated corpus-sweep entries, and the `round_441_lf_deltas_fixture_flags_pin_update_then_persist` regeneration guard (round 441)

- **loop-filter delta election** — the chain-framing sequence encoders elect the six §6.2.8 / §8.8.1 delta axes (`loop_filter_ref_deltas[ 4 ]`: INTRA / LAST / GOLDEN / ALTREF; `loop_filter_mode_deltas[ 2 ]`: `ZEROMV` class vs other inter modes) on every P-frame, after the `(level, sharpness)` election: §8.8.1 derives each block's strength as `lvlSeg + (ref_deltas[ ref ] << nShift) + (mode_deltas[ mode ] << nShift)`, so mixed content whose block classes want *different* strengths — static `ZEROMV` regions any smoothing degrades next to moving `NEWMV` regions the filter helps — reaches per-class strengths the single frame level cannot express. Bounded coordinate descent (6 axes x ≤11 candidates, each a full §8.8 replay) accepting only **strict SSE wins**, because unlike level/sharpness a moved slot costs rate (a §6.2.8 update is 1 + 7 bits); only slots that moved off the §7.2.8 persistent baseline are coded, and the writer threads that baseline across the chain exactly as the decoder's persistent fold does (`LfDeltaState`: keyframes reset it per §7.2 `setup_past_independence( )`, coded updates persist to every later frame until the next update). Measured on a mixed static/moving probe at `q = 170`, level 32: the election moves `ref_deltas[ LAST ]` 0 → +16 and `mode_deltas[ ZEROMV ]` 0 → −16, SSE 350803 → 349178; through the public chained GOP a mid-GOP frame codes `mode_deltas[ 0 ] = +8` and the following frame codes **no** update while filtering with the persisted value. Pinned by a decoder-mirror test at hand-coded deltas *plus* a no-update successor frame (sample-exact through `decode_vp9_sequence` — any §7.2.8 model mismatch desyncs), never-regress/strict-win/determinism election properties, and **black-box validation** of the delta-electing GOP (byte-exact through the reference decoder, persistence included). Classic-framing paths keep their exact historical bytes; per-segment `SEG_LVL_ALT_L` election stays out of scope while the lossy encoders code single-segment frames (round 441)

- **keyframe skip election** — the r434 "later milestone" closes: `encode_keyframe_lossy_tree_elect_skip_with_state` flips every leaf whose entire quantized residual (all coded blocks, all three planes) is zero to `skip = 1`. §6.4.2: a skip block codes no `residual( )` syntax at all, while a non-skip all-zero block still pays the §6.4.24 EOB-at-zero token per coded block — so the election is a **strict rate win at bit-exact identical reconstruction** (§8.6.2 adds the zero residual either way, and — unlike the inter §6.4.11 `read_tx_size( !skip || !is_inter )` gate — the keyframe's §6.4.6 `intra_frame_mode_info( )` calls `read_tx_size( 1 )`, so the planned transform and its §8.5.1 prediction walk are untouched). Wired into the standalone lossy keyframe APIs (all formats — `encode_vp9_lossy` included) and the **chain-framing** sequence encoders; the classic-framing sequence paths keep their exact historical bytes (the staged `lossy-filtered-gop` package pins them). Measured: a flat 256x192 keyframe drops 59 → 36 bytes (−39%, 704/768 MIs elected) and a half-flat 64x48 probe 675 → 673 with the flat half elected — never larger, pinned by identical-reconstruction + strict-shrink + decoder-mirror + both-streams-decode-identically tests, a dense-content no-op pin (no elected leaf ⇒ byte-identical), a chained-vs-classic keyframe rate pin, and black-box validation of a skip-elected stream (byte-exact through the reference decoder) (round 441)

- **lossy sequence encoding across the full §7.2 format matrix** — [`encode_vp9_lossy_sequence_444`] / [`encode_vp9_lossy_sequence_422`] (8-bit profile 1) and [`encode_vp9_lossy_sequence_hbd`] / [`encode_vp9_lossy_sequence_hbd_422`] (10/12-bit profiles 2/3) bring the whole lossy GOP pipeline — content-adaptive keyframe tree, per-block inter tx selection, quarter/eighth-pel motion search, LAST/GOLDEN election, per-leaf skip election, per-frame §8.8 `(level, sharpness)` filter election with filtered reference chaining — to every §7.2.2 format, on the §7.2.6 **chain framing** (shown non-error-resilient P-frames: prev-MV modeling and the `[ LAST, ALTREF ]` compound election live at every format). Internally the two 4:2:0 sequence bodies collapse into one format-generic `encode_sequence_lossy_planes` core (`chained` selects the framing; `lossless_pframe_header_fmt` carries the §6.2 format so every chroma-geometry / quantizer / §8.5.2 MV-rounding derivation keys off it); `encode_vp9_lossy_sequence` / `_chained` are byte-identical wrappers — the staged `lossy-filtered-gop` identity test still passes. Pinned by a 5-format 4-frame translating-GOP sweep through [`decode_vp9_sequence`] (per-frame distortion-regime bounds catch reference-chain drift; byte-determinism), and **black-box validation**: all five GOP streams (plus the nine keyframe streams) decode byte-exact through the reference decoder against this crate's own decode (round 441)

- **lossy keyframe encoding across the full §7.2 format matrix** — [`encode_vp9_lossy_444`] / [`encode_vp9_lossy_422`] (8-bit profile 1) and [`encode_vp9_lossy_hbd`] / [`encode_vp9_lossy_hbd_422`] (10/12-bit profiles 2/3, native `u16` samples) close the encoder's "lossy is 8-bit 4:2:0" gap on keyframes: the content-adaptive tree planner, the decoder-mirror tree encoder, and the §8.8 `(level, sharpness)` filter election were already format-generic (they thread `subsampling_x` / `subsampling_y` / `BitDepth` from the §6.2 header — the §6.4.22 `get_uv_tx_size` chroma transform derivation, the §8.6.1 per-depth quantizer tables, the §6.4.26 per-depth CAT6 token clamp, and the §8.7 transforms all keyed off them), so the new surface is the §6.2.2 `color_config( )` header plumbing (`LossyFormat`: the §7.2 profile is derived from the depth/subsampling pair) plus format-aware plane unpacking. Pinned by a 9-format decoder-mirror sweep (4:2:0 / 4:4:4 / 4:2:2 / 4:4:0 at 8-bit, 10/12-bit at 4:2:0 and the non-420 geometries, partial-MI 41x29 frames — decoded output equals the encoder's in-loop reconstruction sample-exact on all planes), public-path determinism + per-depth distortion-regime bounds, entry-point rejection sweeps, and **black-box validation**: all nine matrix keyframe streams decode byte-exact through the reference decoder against this crate's own decode (env-gated `dump_matrix_streams_for_black_box_validation` emits the IVF + crate-decode pairs). `encode_vp9_lossy` / the 4:2:0 primitives are byte-identical wrappers over the generalised path — every staged-fixture identity test still passes (round 441)

- **[`encode_vp9_lossy_sequence_chained`] — the lossy GOP on the §7.2.6 chain model, with compound LIVE in ordinary shown GOPs** — `encode_pframe_lossy_tree_motion_with_state` gains the `prev_frame_mvs` model: the previous frame's §6.4.4 motion field feeds every §6.5 predictor derivation in the search (the leaf-level candidate evaluation, the compound second-list re-snap, the final NEARESTMV/NEARMV mode mapping, and the sub-8x8 cell walk) plus the block writer, so shown non-error-resilient lossy chains code with predictors bit-identical to the decoder's `UsePrevFrameMvs == 1` scan. Because a non-error-resilient frame keeps its CODED sign biases (§7.2 `setup_past_independence` only zeroes them on error-resilient frames), the asymmetric-ALTREF bias the sequence encoder always wrote — dead under the error-resilient chain — now genuinely admits `[ LAST, ALTREF ]` compound: the r418 restriction (compound needed a hidden/intra predecessor) dissolves, pinned by a cross-fade-midpoint GOP whose election codes compound leaves on the writer's own §6.4.4 state with every frame shown, decoding midpoint-accurate. The classic `encode_vp9_lossy_sequence` / `_rc` paths pass no prev field and stay byte-identical — every staged-fixture identity test still passes (round 434)

- **`encode_chained_sequence` fuzz target + in-CI deterministic smoke** — a second oracle-carrying harness over the §7.2.6 chain model: fuzz-derived `(width 1..=48, height 1..=48, 2..=4 frames, cycled shifted content)` runs `encode_vp9_lossless_sequence_chained` → `decode_vp9_sequence` with the **full lossless oracle** (every decoded frame must equal its input byte-exact — strictly stronger than "must decode": a writer/decoder prev-field mismatch desyncs the §6.5 predictors and shows up as byte divergence). A 24-case LCG-deterministic smoke of the same body (`chained_sequence_smoke_over_random_geometries`) runs in standard CI so arbitrary partial-MI geometries exercise the chain model on every push; the fuzz sub-crate builds clean against the new API (round 434)

- **[`encode_vp9_lossless_sequence_chained`] — the lossless sequence encoder on non-error-resilient chain framing** — the first production consumer of the §7.2.6 `UsePrevFrameMvs == 1` writer model: every P-frame is shown and non-error-resilient (the decoder scans the previous frame's motion field), and the encoder threads each frame's §6.4.4 write-back into BOTH the next frame's motion-search predictor derivation and its block writer (`encode_pframe_lossless_motion_prev`, with `PrevMotionField::after_intra_frame` modeling the keyframe's all-intra field — pinned equal to the keyframe assembler's actual returned state). A vector the previous frame already coded at the same position reaches the §6.5.10 prev candidate list and maps to `NEARESTMV` / `NEARMV` (no §6.4.20 mv-diff bits), so temporally persistent motion that spatial neighbours mispredict codes fewer bytes: on a 64x128 banded-motion probe (per-8px-band velocities cycling +2/-2/+4/-4 px/frame) the chained P-frames code 1829 B vs the error-resilient chain's 1845 B, deterministic and strict, with both chains byte-exact. The classic `encode_vp9_lossless_sequence` and `encode_pframe_lossless_motion` paths are byte-identical to before (wrappers over the new `_prev` / `_with_state` internals with no prev field) — every staged self-encoded fixture's identity test still passes (round 434)

- **§7.2.6 `UsePrevFrameMvs == 1` writer model — previous-frame-MV modeling, the r418 "later milestone"** — `InterFrameTreePlan` gains `prev_frame_mvs: Option<PrevMotionField>`: the previous decoded frame's §6.4.4 `RefFrames` / `Mvs` write-back arrays (exactly what the `_with_state` assemblers return), fed to the §6.5.10 candidate scan through the same shared `FrameStateMvSource` the decoder reads — so the `NearestMv` / `NearMv` / `BestMv` predictors are bit-identical when the decode-side derivation is 1 (shown same-sized predecessor, non-error-resilient header). Non-`ZEROMV` blocks on non-error-resilient headers no longer require an error-resilient header or a hidden/intra predecessor: shown P-frame chains — **and therefore compound prediction without a hidden predecessor** — are now codeable. Coherence is validated at every level (the prev field on an error-resilient header, alongside the `prev_frame_mvs_absent` assertion, or mis-sized is rejected; the block writer rejects a `use_prev_frame_mvs` flag/field mismatch). Pinned by: a keyframe → P1 → P2 shown non-ER chain where P2 codes `NEARMV` with a vector whose ONLY §6.5.10 source is the prev-frame pass (the identical plan without the field is `Unsupported` — the differential proof the field feeds the predictors), sample-identity of that chain against an error-resilient twin coding the same final vectors as `NEWMV` (the §8 reconstruction is model-invariant while the entropy paths differ; the prev chain decodes through the decoder's real §7.2.6 derivation, so any model mismatch desyncs), a compound `[ LAST, ALTREF ]` frame on a SHOWN chain whose every `ZEROMV` sample equals the independently computed `Round2( LAST + ALTREF, 1 )` average, and the up-front rejection sweep (round 434)

- **multi-tile frame WRITERS — the tree assemblers accept any §6.2.13 tiling** — `assemble_keyframe_tree` / `assemble_inter_frame_tree` (and their `_with_state` variants) drop the `tile_cols_log2 == tile_rows_log2 == 0` guard and mirror the §6.4 `decode_tiles( )` row-major walk exactly: one fresh §9.2 coder bracket per tile, f(32) big-endian `tile_size` prefixes on every tile but the last (§6.4 — the last tile's size is implied), the four `get_tile_offset( )` extents per tile, above-context strips (partition / nonzero / seg-pred) carrying ACROSS tiles with the left strips resetting per superblock row *including each tile's first* (§7.4.2 — a right-hand tile never reads context its left neighbour wrote), the §6.5 candidate scans clamped to each tile's `MiColStart` / `MiColEnd` window, and the §6.4.4 `AvailL = MiCol > MiColStart` clamp at column edges. Pinned by: reconstruction-IDENTITY across tile-row splits at the staged fixture's 2col x 4row tiling (tile rows never change the §8 reconstruction — `AvailU` is the frame-wide `MiRow > 0` — while the entropy layout changes completely, so identity pins the whole tile walk), a degenerate EMPTY tile row (64x192 at `tile_rows_log2 = 2`: `MiRowStart == MiRowEnd`, the writer emits a valid empty §9.2 bracket), an all-inter tiled P-frame with `NEWMV` leaves along the tile boundaries decoding identically to its single-tile assembly (plan-dictated MVs make inter reconstruction tiling-invariant; the per-MI write-back state matches too), a pinned reconstruction *difference* across a column split (intra `AvailL` genuinely vanishes at the boundary — the writer must model the decoder's clamp, not predict across it), and the §6.4.1 tile-size walk consuming the tiled frame's bytes exactly (round 434)

- **staged `tiles-2col-4row-inter` fixture consumed — tile ROWS corpus-validated, 43/43 byte-exact** — the corpus's first `tile_rows_log2 > 0` stream (512x256, `tile_rows_log2 = 2` AND `tile_cols_log2 = 1` on both the keyframe and the P-frame: `Sb64Cols = 8` / `Sb64Rows = 4`, so the §6.4.1 row-major tile walk runs 4x2 = 8 genuine tile payloads per frame — eight §9.2 coder brackets, per-tile above/left context resets interacting with the row traversal, and intra + §8.5.2 inter prediction across tile-row boundaries) decodes **byte-exact with no code change**: the §6.4 tile-row walk was already correct, and the long-standing "custom encoder tooling needed" ledger entry is closed (the black-box encoder clamps tile rows to 0 above one thread; a single-threaded encode emits them — see the fixture's notes.md). The stream joins the whole-corpus byte-exact sweep (presence-gated, 43/43) and a `round_434_tile_rows_fixture_flags_pin_row_split` test pins `tile_rows_log2 = 2` / `tile_cols_log2 = 1` on both frames so a regenerated stream can't silently drop the row split; the now-staged round-420 `lossy-filtered-gop` package is likewise picked up by its presence-gated sweep/flags/identity tests (round 434)

- **`encode_lossy_keyframe` fuzz target — the first oracle-carrying harness** — round-trips fuzz-derived `(width 1..=96, height 1..=96, base_q_idx, cycled content)` through `encode_vp9_lossy` → `decode_vp9`: unlike the pure panic-surface targets, a stream the encoder DID emit must decode (the self-encoded frame runs this crate's full §8.1 chain, elected §8.8 filter included; any `Err` is an encoder bug). A 300-case deterministic smoke of the body passed locally (298 encoded+decoded, 2 correctly-rejected `q == 0`) across random partial-MI geometries and elected filter parameters (round 420)

- **`loop_filter_sharpness` election + the standalone lossy keyframe API elects too** — the election covers both free §6.2.8 axes: `elect_filter_params` runs a bounded two-stage search (the 0..=63 level sweep at the header's sharpness, then the 0..=7 sharpness sweep at the winning level — the §8.8.4 `Clip3( 1, 9 - sharpness, lvl >> shift )` limit derivation trades smoothing strength for edge-detail retention; stage 2 moves only on a strict SSE win, ties keeping the header's sharpness), and both sequence encoders code the elected pair. `encode_keyframe_lossy_420` (public `encode_vp9_lossy`) now closes out through the same election — a standalone keyframe's decoded output IS its filtered reconstruction, so the election lifts display quality at zero rate cost (a 72x56 partial-MI probe elects level 21 and stays byte-exact through the black-box reference decoder); the `_with_recon` primitives stay level-0 for the sequence/fixture callers that run their own election. Encode-side sharpness pinned decoder-mirror sample-exact at sharpness 0/3/7 (with the 0-vs-7 outputs demonstrably different), and the stage-2 never-regress property is test-pinned (round 420)

- **filtered-GOP A/B pin + self-encoded `lossy-filtered-gop` fixture** — the encode-side §8.8 election's rate/quality evidence is test-pinned: on gently-graded translating content at `base_q_idx = 140`, the sequence encoder elects keyframe level 49 and cuts the whole decoded GOP's SSE from 85340 to 78850 (−7.6% distortion) at an **identical 443-byte total rate** vs the same chain with filtering forced off (the §6.2.8 level field is fixed-width, so the election is rate-free on the frame that elects it). `build_lossy_filtered_gop_stream( )` emits the corpus's first stream whose non-zero `loop_filter_level`s are elected by this crate's own encoder (64x64, 4 frames through the public `encode_vp9_lossy_sequence`, per-frame elected levels 19/27/19/24; the existing non-zero-level streams are black-box encodes): byte-deterministic, every header pinned to carry a non-zero level, staging generator + docs-gated staged-bytes identity + presence-gated corpus-sweep/flags-pin wiring, and the staged package (input.ivf + a black-box reference decode for expected.yuv) verified **byte-exact** through the reference decoder against the crate's own decode — including two extra local black-box shapes (a 76x52 partial-MI fixed-q GOP whose filter crosses the visible edge, and a rate-controlled GOP), all byte-exact (round 420)

- **encode-side §8.8 loop filter + per-frame filter-level ELECTION in the lossy sequence encoders** — the encoder's reconstruction path now runs the identical §8.8 chain the decode path runs (the §8.8.1 `loop_filter_frame_init( )` over the §7.2-resolved deltas, then the §8.8 superblock raster) on its own reconstruction, keyed by the writer's final §6.4.4 per-MI arrays: the assemblers gain `_with_state` variants returning the `Vp9FrameState` write-back arrays (bit-identical to the decoder's, and exactly the §8.8.2/§8.8.4 per-MI input), `recon_filter::filter_reconstruction` replays §8.1 step 2 on a `ReconState`, and `recon_filter::elect_filter_level` sweeps every `loop_filter_level` candidate `0..=63` (free encoder choice; the §6.2.8 level field is fixed-width, so the choice never changes the stream length) scoring the filtered reconstruction's SSE against the source over the visible extents, ties resolving to the smaller level. `encode_sequence_lossy_420` / `encode_sequence_lossy_rc_420` (public `encode_vp9_lossy_sequence` / `_rc`) close out every frame — keyframe included — through the election: the coded header carries the elected level and the reference chain threads the **filtered** reconstructions, mirroring the §8.10 post-filter `FrameStore[ ]` every conforming decoder keeps (an encoder that codes a non-zero level but references its unfiltered recon drifts on the next inter frame). Decoder-mirror pinned sample-exact on all three planes for keyframes (the §8.8.1 INTRA ref-delta path) and P-frames (the inter ref/mode-delta paths) at multiple levels, election properties pinned (exact recon elects 0; coarse-q textured content elects non-zero and strictly reduces SSE; deterministic), and the sequence chain test replays the election end-to-end (round 420)

### Changed

- **internal test-only public surface marked `#[doc(hidden)]`** — the crate-root re-exports of the header / compressed-header parsers, coefficient-probability tables, and §8.8 loop-filter leaf primitives (`filter_size`, `adaptive_filter_strength`, `filter_mask`, `narrow_filter`, `wide_filter`, `sample_filtering`, the superblock filter/loop-filter helpers, plus `parse_uncompressed_header` / `parse_compressed_header` and their types) are exposed only for the in-crate tests and fuzz targets; they are now `#[doc(hidden)]` so API tooling stops tracking them as stable surface. The stable API is unchanged: the `decode_vp9` / `decode_vp9_sequence` / `decode_intra_frame` decoders, the pixel-accurate `encode_vp9*` encoders, `split_superframe`, `register`, and the `Vp9DecodedFrame` / `Error` types stay visible. Docs, comments, and attributes only — no signature or visibility changes

### Fixed

- **§7.2 `setup_past_independence( )` was missing two resets: `ref_frame_sign_bias` and `PrevSegmentIds`** — on a key / intra-only / **error-resilient** frame, §7.2 zeroes `ref_frame_sign_bias[ i ]` for all i *after* the §6.2 ref-frames loop consumed the sign-bias bits (the coded values are dead), and clears `PrevSegmentIds[ ][ ]` to zero. The decoder now applies both (alongside the feature-table / loop-filter-delta resets it already modeled): an error-resilient inter frame's effective sign biases are all zero — which makes §6.3.12 `compoundReferenceAllowed` 0 (an error-resilient frame can never code compound prediction) and disables §6.5 candidate sign flips — and its §6.4.14 predictor map is the zero map. The writer side mirrors the semantics: `assemble_inter_frame_tree` derives every bias-dependent value (compound layout, §6.5 scans, the §6.3.12 gate) from the *effective* biases, rejects a non-single `reference_mode` and temporal seg-map updates on error-resilient headers, and now accepts hidden (`show_frame = 0`) inter frames plus a `prev_frame_mvs_absent` plan assertion so a **non**-error-resilient frame with a hidden / intra / resized predecessor (§7.2.6 `UsePrevFrameMvs == 0`) can carry non-`ZEROMV` blocks — the only header shape on which compound is codeable. Found by black-box reference-decoder divergence on the first self-encoded compound stream (the r413 compound election path had never been black-box-validated; every decoder-side compound read was — `superframe-2` is a `REFERENCE_MODE_SELECT` stream — but the writer-side combination "error-resilient + compound" produced streams the reference decoder parses as single-reference). The `lossy-compound-elected` fixture is rebuilt as a valid stream (hidden predecessor + `show_existing_frame` + non-error-resilient compound frame, 26x rate win intact, 41/41 corpus byte-exact); the compound crossfade and compound-sub-8x8 tests restructured the same way, and error-resilient guard rejections are pinned (round 418)


- **§8.8.2 loop-filter views passed at the visible extent instead of the MI-aligned working extent** — the §8.8.2 step-13 `onScreen` predicate tests `x >= 8 * MiCols` / `y >= 8 * MiRows` (the mode-info grid), NOT the visible `FrameWidth` / `FrameHeight`: on a frame whose dimensions are not multiples of 8, edges *between* the visible boundary and the MI-grid boundary are on-screen and are filtered, and the §8.8.5.1 stencil reads the **real** reconstructed samples beyond the visible edge (§6.4 decodes whole mode-info blocks, overhang included). `decode_single_frame` handed the filter `SuperblockFilterPlane` views sized to the visible crop, so those q-side reads were edge-replicated and the corresponding writes dropped — silently desynchronising every visible row/column the filter touches from across the frame edge (§8.10 crops *after* filtering, so only visible samples showed it). Found root-causing the new `partial-mi-58x36-yuv444` fixture (3 differing V-plane samples on the bottom visible row of every frame, keyframe included: the horizontal edge at `y == FrameHeight == 36` filters with rows 36..39 as its q side); the views now carry the MI-aligned working extents, matching the module's documented contract, and the whole corpus — including both new partial-MI streams — decodes byte-exact (round 412)

### Added

- **§6.4.12 temporal-predicted segment-id writer branch — the last §6.4.11 writer arm** — `write_inter_block` codes the temporal-update path of `inter_segment_id( )`: the §6.4.14 spatial-min predictor is derived over the caller-supplied `PrevSegmentIds[ ][ ]` plane (`InterBlockFrameCtx::prev_segment_ids`), `seg_id_predicted` is written under `segmentation_pred_prob[ ctx ]` with the §9.3.2 `LeftSegPredContext[ MiRow ] + AboveSegPredContext[ MiCol ]` ctx (flag = 1 exactly when the planned id equals the predictor — the shortest encoding; only a non-predicted id pays the §6.4.7 tree escape), and the §6.4.12 trailing ctx-strip write-back runs over the block's `bw` / `bh` through the same shared `SegPredContextState::write_back` the decoder now uses (deduplicated out of the §6.4.12 decode driver). `assemble_inter_frame_tree` threads the state: `InterFrameTreePlan` gains `prev_segment_ids`, the writer-side strips reset per §7.4.1/§7.4.2 (above per tile, left per superblock row, mirroring `decode_tile`), and a temporal-update header without a correctly-sized prev map is rejected up front. Round-trip-pinned through the real §6.4.12 path-4 decode walk: predicted 8x8 + 32x32 blocks (the min-over-region predictor), tree-escape blocks, ctx strips carrying between blocks on both sides, and all four missing-input rejections (round 418)

- **self-encoded `temporal-seg-predicted` fixture (39/39 corpus)** — the corpus's first stream whose temporal seg-map frames are *writer* output with hand-planned predicted/escape placement (the black-box encoders only emit `segmentation_temporal_update = 1` through AQ heuristics with uncontrollable block placement): `build_temporal_segmap_stream( )` codes a keyframe (pattern A), a hidden intra-only frame (pattern B → slot 1), a P-frame establishing a visible two-segment map non-temporally (a `SEG_LVL_REF_FRAME = GOLDEN` band on MI rows 0..3 — band blocks reconstruct pattern B), two **temporal-update** P-frames shifting the band (rows 1..4 then 2..5: unchanged rows code `seg_id_predicted = 1` against §6.4.14 `PrevSegmentIds`, the two changed rows per frame code the §6.4.7 tree escape, with the §7.4 ctx strips threading across each frame), and a level-0 copy frame pinning the §8.10 store. In-crate pins: all five shown frames decode byte-exact against the band composites (the map semantics are *visible* through the GOLDEN override), the F3/F4 headers carry `temporal_update = 1` + the coded `segmentation_pred_prob`, the stream is byte-deterministic, and a docs-gated identity test asserts the staged fixture bytes ARE the builder's output; `expected.yuv` is a black-box reference decode, the stream joins the whole-corpus byte-exact sweep (39/39), and a `round_418_fixture_flags_pin_their_feature_classes` test pins the header classes (round 418)
- **sub-8x8 ELECTION in the lossy partition search** — the content-adaptive planner (`plan_inter_partitions`) now probes each 8x8 MI cell's four 4x4 quadrants with independent integer full searches and elects a **below-8x8 leaf** where divergent quadrant motion beats the cell's best single-vector SAD by a per-coded-vector syntax margin: `PARTITION_VERT` (4x8) when only the left/right halves differ, `PARTITION_HORZ` (8x4) when only the top/bottom halves differ, `PARTITION_SPLIT` (4x4) otherwise — and a cell with an elected split never merges upward. Elected leaves flow through `encode_pframe_lossy_tree_motion` as real §6.4.16 per-`(idy, idx)` walks (per-cell `NEWMV` / `ZEROMV` over `LAST`, each vector snapped §6.4.20-codeable against the leaf's §6.5.12 `BestMv` under the §6.5.13 `use_mv_hp` gate), predict via the decoder-mirror per-`blockIdx` §8.5.2 walk, and run the same trial-quantization skip / strict-SSE-improvement guard as `>= 8x8` leaves at the forced `TX_4X4`. The r415 sub-8x8 writer branches are now *searchable*, not just plannable — `encode_sequence_lossy_420` / `_rc_420` run with the probe on. Rate/quality evidence pinned by test: on content with opposing quadrant motion inside 8x8 cells, the sub-8x8-enabled encode is **56 B with SSE 0** against the target where the 8x8-limited encode needs **147 B with SSE 957** — smaller *and* strictly closer — with the coded stream decoding byte-exact against the encoder's reconstruction; the planner election (VERT / HORZ / SPLIT shape + per-quadrant vectors, and no election with the probe off or on uniform motion) is pinned by a dedicated planner test (round 418)
- **self-encoded `lossy-sub8x8-elected` fixture (40/40 corpus)** — the corpus's first stream whose sub-8x8 inter blocks are placed by the encoder's **own search** (the hand-planned `sub8x8-inter-mvs` fixture pins the writer branches; this one pins the election path): a lossless keyframe of an alias-free hash texture, then a lossy P-frame whose content carries opposing 4x4-quadrant motion in three 8x8 cells, so the partition probe elects all three below-8x8 shapes (VERT 4x8, HORZ 8x4, SPLIT 4x4) with per-cell `NEWMV` vectors that predict exactly (the elected leaves skip), plus a copy frame pinning the §8.10 store. In-crate pins: the decoded P-frame equals the displaced target sample-exact, byte-determinism, the docs-gated staged-bytes identity test, header flags (error-resilient lossy `base_q_idx = 80` P-frame over a lossless keyframe), and the whole-corpus byte-exact sweep at **40/40**; `expected.yuv` is a black-box reference decode (round 418)
- **sub-8x8 inter writer branches — the §6.4.16 per-`(idy, idx)` MV walk** — `write_inter_block` accepts `MiSize < BLOCK_8X8` blocks: each visited cell writes its own `inter_mode` token (the block-level `ModeContext` row, per §9.3.2) and §6.4.18 `assign_mv( )`, with the §6.5.14 `append_sub8x8_mvs( )` predictor rewrite for `NEARESTMV` / `NEARMV` cells (the block-0 candidate pair, the block-1/2 seed from `BlockMvs[ 0 ]`, and the block-3 seed from `BlockMvs[ 2 ]` with the idx-1..0 dedup walk — shared decode code, so the predictors are bit-identical), replicating each cell's vector across its `(num4x4h × num4x4w)` span into `BlockMvs` exactly as the decoder does. `InterBlockSpec` gains a per-cell `sub` plan (`InterSubBlockSpec`: modes + per-list MVs); the §6.4.3 tree assembler's `InterTreeLeaf` carries it through `assemble_inter_frame_tree`, so partition layouts may now split `BLOCK_8X8` nodes into 4x4 / 4x8 / 8x4 leaves (single-leaf dispatch per the §6.4.3 sub-8x8 arm), including non-skip leaves coding the §6.4.21 inter residual at the `Max( MiSize, BLOCK_8X8 )` grid. Round-trip-pinned through the real §6.4.11/§6.4.16 decode walk (per-cell NEWMV, the NEARESTMV/NEARMV seeds, compound sub-8x8, 4x8/8x4 replication, non-skip residual) and end-to-end through `decode_vp9_sequence`; uncodeable plans (missing sub spec, non-TX_4X4 tx, SEG_LVL_SKIP on a sub-8x8 shape, NEWMV cells without an error-resilient header) are rejected (round 415)
- **lossless layout P-frame encoder + self-encoded `sub8x8-inter-mvs` fixture (37-fixture corpus)** — `encode_pframe_lossless_layout` encodes a lossless P-frame over a caller-chosen §6.4.3 layout, predicting every leaf with exactly the vectors it codes — sub-8x8 leaves via a decoder-mirror per-`blockIdx` §8.5.2 walk (the per-plane 4x4 grid of the 8x8 MI cell; the shared driver resolves per-cell luma vectors and averaged sub-8x8 chroma vectors) — and coding the §8.7.2 WHT residual so `prediction + residual == target` holds sample-exact. `build_sub8x8_inter_stream( )` + an env-gated staging test emit the corpus's first stream with *planned* per-sub-block motion (no black-box encoder wrapper exposes it): six sub-8x8 nodes (two SPLIT → 4x4, two HORZ → 8x4, two VERT → 4x8) carrying NEWMV (integer + quarter-pel), NEARESTMV, NEARMV and ZEROMV cells with live sub-8x8 WHT residual, every other block an exact-copy ZEROMV skip, plus a copy frame pinning the §8.10 store. In-crate pins: byte-exact decode against the lossless composite targets, byte-determinism, and a docs-gated identity test asserting the staged fixture bytes ARE the builder's output; `expected.yuv` is a black-box reference decode and the stream joins the whole-corpus byte-exact sweep (round 415)
- **compound + 4:4:4 / 4:2:2 / 4:4:0 / 10-bit sub-8x8 verified end-to-end** — `encode_pframe_lossless_layout` gains an optional second reference: leaves pairing `[ LAST, ALTREF ]` (the §6.3.18 fixed/variable layout under asymmetric-ALTREF sign biases) predict compound — the sub-8x8 per-`blockIdx` walk included, so the §8.5.2 `Round2( p0 + p1, 1 )` average runs per 4x4 cell — and the frame codes `reference_mode = SELECT`. Byte-exact end-to-end pins: a compound sub-8x8 P-frame (two-list per-cell NEWMV, compound NEARESTMV via the per-list §6.5.14 seeds, a full compound 8x8 leaf) drawing LAST from the keyframe and ALTREF from a hidden intra-only slot; a **4:4:4** sub-8x8 P-frame (full-resolution chroma reads each 4x4 cell's own `blockIdx` vector — no sub-8x8 MV averaging, a distinct §8.5.2 arm from 4:2:0); a **4:2:2 / 4:4:0** pair (half-resolution chroma on one axis averages exactly *two* luma cell vectors per chroma 4x4 — the third averaging arm), a **10-bit** profile-2 sub-8x8 P-frame (bit-depth-scaled convolution clamps + HBD-range WHT residual), and a switchable per-block `interp_filter` on a sub-8x8 block (token order between the absent block-level `inter_mode` and the per-cell walk) (round 415)
- **self-encoded `render-size-128x72` fixture — the §6.2.3 `render_and_frame_size_different = 1` arm (38/38 corpus)** — the last fixture-less signalling axis of the encoder-tooling-gap ledger closes: every coded frame of the stream writes the explicit `render_width_minus_1` / `render_height_minus_1` pair (64x64 coded, 128x72 render) across all three `render_size( )` call sites — the §6.2 key-frame arm (F0, pattern A), the intra-only arm (F1, hidden, pattern B → slot 1), and the §6.2.5 `frame_size_with_refs( )` explicit-size inter arm (F2 over the intra-only slot, F3 copy frame). The black-box encoder wrappers only signal display geometry through container-level aspect metadata (their bitstream render fields stay 0), so the axis is only mintable by the in-crate writers (`build_render_size_stream( )` + env-gated staging + docs-gated staged-bytes identity test). In-crate pins: 3 shown frames byte-exact against the source patterns and every parsed header carrying the 128x72 override; `expected.yuv` is a black-box reference decode, and a `round_415_fixture_flags_pin_their_feature_classes` test asserts both new fixtures' header classes (render override on all four frames; error-resilient lossless sub-8x8 P-frame) (round 415)

- **§6.2 intra-only + hidden-inter header-writer branches; intra-only frames self-encodable** — `write_uncompressed_header` gains the intra-only branch (`show_frame = 0`, `intra_only = 1`: the explicit flag, `reset_frame_context`, mid-stream `frame_sync_code`, the `Profile > 0` `color_config( )` / profile-0 implicit BT.601-4:2:0-8-bit validation, `refresh_frame_flags f(8)`, explicit `frame_size` + `render_size`) and the hidden-inter case (`show_frame = 0` inter frames code an explicit `intra_only = 0` flag). A *shown* header claiming `intra_only` is rejected (§6.2 only reads the flag when `show_frame == 0`). The keyframe assemblers (`assemble_keyframe` / `assemble_keyframe_tree`) and `encode_keyframe_lossless` now accept hidden intra-only headers — §6.4.5 `mode_info( )` dispatches on `FrameIsIntra`, and the §9.3.2 partition / §6.4.6 mode probabilities key on `FrameIsIntra`, not `frame_type`, so both intra classes code the identical §6.3/§6.4 body. `build_intra_only_stream( )` + an env-gated staging test emit the corpus's `intra-only` fixture byte-deterministically: keyframe (pattern A) → **hidden intra-only frame** (pattern B → slot 1, `reset_frame_context = 3` — the corpus's first non-zero reset, resetting all four §6.1.2 banks mid-stream) → `show_existing_frame` of slot 1 → all-skip `ZEROMV` P-frame whose `LAST` is slot 1. In-crate pin: 3 shown frames decode byte-exact against the *source patterns* (A, B, B — an independent oracle); the staged fixture's `expected.yuv` is a black-box reference decode of the same stream (round 412)

- **self-encoded odd-luma stream — `odd-dims-59x37`** — truly odd luma dimensions in both axes (chroma 30x19 per the `(w + 1) >> 1` rounding): the common black-box encoder pipelines round input dimensions to even, so the stream is emitted by `build_odd_dims_stream( )` (public `encode_vp9_lossless_sequence` — keyframe + 3 lossless P-frames of diagonally-translating content with real `NEWMV` motion at the odd frame edges) + an env-gated staging test, and the staged fixture's `expected.yuv` is a black-box reference decode (the reference decoder emits the same odd geometry). In-crate pin: 4 frames decode byte-exact back to the source content at exactly 59x37 (round 412)

- **whole-corpus sweep at 35/35 byte-exact — corpus extension x9, round-412 flags pin** — the round-412 fixtures join `full_corpus_sequences_byte_exact`: `odd-dims-59x37` (truly odd luma, self-encoded), `profile-1-yuv440-8bit-inter` (4:4:0 — the §8.5.2 y-only chroma MV rounding + subY-only §6.4.22/§8.8 chroma geometry, previously untested), `profile-2-yuv420-12bit-inter` + `profile-3-yuv422-12bit-inter` (12-bit **inter** — CAT6 18-bit tokens through motion compensation + §8.4 adaptation; closes the HBD x chroma matrix), `cyclic-refresh-aq` (**`segmentation_temporal_update=1`** P-frames — the §6.4.12 `seg_id_predicted` branch over §6.4.14 `PrevSegmentIds` corpus-validated for the first time, plus inter-frame `SEG_LVL_ALT_Q` rewrites), `tiles-4col-inter` (`tile_cols_log2=2`: four §9.2 coder brackets + three interior tile-boundary context resets per frame), `color-bt709-full-range` (CS_BT_709 + full-swing range signalling), `rgb-8bit-inter` (CS_RGB on an inter GOP), and `intra-only`. A `round_412_fixture_flags_pin_their_feature_classes` test asserts each stream's headers actually carry its feature class — chroma geometry, bit depth, tiling, color signalling, temporal-seg-map frames (with `segmentation_pred_prob` present), and the intra-only packet layout (round 412)

- **segment-feature-aware inter block writer** — `write_inter_block` (and the §6.4.3 inter tree assembler, whose `InterTreeLeaf` gains a per-leaf `segment_id`) now models the §6.4.9 `seg_feature_active` no-bits arms: on a `SEG_LVL_SKIP` segment §6.4.8 hardwires `skip = 1` (no skip bit) and §6.4.16 forces `y_mode = ZEROMV` (no `inter_mode` token); on a `SEG_LVL_REF_FRAME` segment §6.4.13 derives `is_inter` and §6.4.17 derives the reference pair from `FeatureData` (no bits for either) — the writer validates the plan against the feature table (a non-skip plan on a SKIP segment, or a mismatched reference on a REF segment, is uncodeable) and otherwise skips exactly the elements the decoder never reads. `InterBlockFrameCtx` carries the full §6.2.11 `SegmentationParams`; the assembler threads the header's `segmentation_tree_probs` so a live (non-temporal) `update_map` codes each leaf's segment id through the §6.4.7 tree. Round-trip-pinned through the real §6.4.11 decode walk: SKIP-segment and REF-segment blocks recover their forced skip / ZEROMV / derived reference with no bits coded, and both uncodeable plans are rejected (round 412)

- **self-encoded segment-features stream — `seg-features-skip-ref-altl` (36/36 corpus)** — the three segmentation features no black-box encoder emits (`SEG_LVL_SKIP` / `SEG_LVL_REF_FRAME` / `SEG_LVL_ALT_L` — only `SEG_LVL_ALT_Q` appears in the AQ modes), plus the corpus's first **non-zero `loop_filter_sharpness`** (the §8.8.4 `shift` / clipped-`limit` derivation): `build_seg_features_stream( )` codes a keyframe (pattern A), a hidden intra-only frame (pattern B → slot 1), an all-skip P-frame with a live four-segment map striped by superblock row (`LAST` = A, `GOLDEN` = B; segment 1 SKIP-forced, segment 2 REF-overridden to GOLDEN, segment 3 ALT_L +32; `loop_filter_level = 16`, `sharpness = 3`), and a level-0 copy frame pinning that the §8.10 reference stores hold **post-§8.8** samples. In-crate pins: the REF-override stripe reconstructs from pattern B and the others from pattern A on filter-untouched interior samples, the §8.8 filter demonstrably ran at the stripe boundary, the copy frame equals the feature frame byte-for-byte, and the stream is byte-deterministic; the staged fixture's `expected.yuv` is a black-box reference decode and the whole stream decodes byte-exact against it (round 412)

- **corpus extension x2 — partial-MI geometry** — `partial-mi-58x36-yuv420` (keyframe + 7 P-frames: non-multiple-of-8 luma on both axes, odd 29-wide 4:2:0 chroma planes, §8.4 backward adaptation live) and `partial-mi-58x36-yuv444` (same geometry at full-resolution chroma — the §8.8.2 regression stream above). Black-box generation + reference decodes, commands + SHA-256 in each fixture's notes.md; both join the whole-corpus byte-exact sweep (round 412)

- **corpus extension x3 — 24/24 byte-exact** — three new black-box-generated stream classes (commands + SHA-256 in each fixture's notes.md): `lossless-inter` (the §8.7.2 lossless WHT on **inter** residuals — prior lossless coverage was keyframe-only), `tiles-2col-inter` (`tile_cols_log2=1` on real P-frames: per-tile §9.2 coder brackets + §7.4 tile-boundary context resets interleaving with §8.5.2 motion compensation; tile **rows** recorded as an encoder-tooling gap — the black-box wrapper emits `tile_rows_log2=0` regardless of its knob), and `hbd-backward-adaptation` (profile-2 10-bit with `frame_parallel_decoding_mode=0`: §8.4 backward adaptation over the §6.4.26 extended-CAT6 high-bit-depth token counts). A flag-pinning test asserts each fixture's headers actually carry its feature class (round 409)
- **§8.5.2.3 scaled-reference decode corpus-validated — 21/21** — the reference-scaling path (an inter frame sampling a live reference of a *different* coded size) graduates from unit tests to corpus validation via the new `scaled-reference` fixture (`docs/video/vp9/fixtures/scaled-reference/`). The #249-part-2 docs entry records that no black-box encoder CLI mints such a stream, so it is **self-encoded** by the in-crate writers (`build_scaled_reference_stream( )` + an env-gated staging test emit it byte-deterministically): a 128x128 lossless keyframe, then all-skip inter frames at 64x64 (reference = exactly **2x** — the §6.2.5 conformance extreme, `stepX = 32`), 128x128 over the 64x64 recon (1/2x upscale, half-pel phases), 96x96 over the keyframe (fractional 4/3 ratio — non-power-of-two `fracX`/`fracY`), and an error-resilient `NEWMV [8, 16]` frame over the 2x reference (§8.5.2.1-§8.5.2.3 MV scaling). `expected.yuv` is a black-box reference decode (`-noautoscale` keeps native per-frame sizes) and the whole stream decodes **byte-exact**; the in-crate test additionally pins two closed-form §8.5.2.3 *phase-0 identities* (2x + zero/even MV ⇒ exact 2:1 decimation of the reference, for both the ZEROMV and the scaled-NEWMV frame) — an independent derivation, not an implementation replay (round 409)
- **§6.1.2 `refresh_probs( )` backward adaptation wired — non-frame-parallel streams decode** — `decode_vp9_sequence` now runs the full §6.1.2 branch when `error_resilient_mode == 0 && frame_parallel_decoding_mode == 0`: `load_probs( idx )` reloads the pre-frame bank (discarding the frame's forward updates on every table except `tx_probs` / `skip_prob`, which `load_probs( )` does not cover), §8.4.3 `adapt_coef_probs( )` folds the frame's `counts_token` / `counts_more_coefs` into the bank's coefficient probabilities (updateFactor 112/128 selected from `FrameIsIntra` and the newly-tracked §7.2 `LastFrameType`), and on inter frames `load_probs2( idx )` + §8.4.4 `adapt_noncoef_probs( )` adapt everything else; `save_probs( idx )` persists the result. Corpus-validated by the new `backward-adaptation` fixture (`docs/video/vp9/fixtures/backward-adaptation/`, the corpus's first `frame_parallel_decoding_mode=0` stream — keyframe + 9 P-frames): **byte-exact through the whole GOP**, where every frame's entropy decode runs on probabilities adapted from the previous frame's counts (negative control: with adaptation disabled the same stream fails to decode at all). Corpus: **20/20 fixtures byte-exact** (round 409)
- **§9.3.4 non-coefficient syntax-element counting** — every remaining counted element of the §9.3.4 table is now accumulated at its decode site with the same ctx the §9.3.2 probability selection used: `partition` (via a new `LeafSink::record_partition` hook), `skip`, `is_inter`, `tx_size`, `intra_mode` / `sub_intra_mode` / `uv_mode`, `comp_mode` / `comp_ref` / `single_ref_p1` / `single_ref_p2`, `inter_mode` (per sub-block in the sub-8x8 walk), `interp_filter`, and the full mv family (`mv_joint`, `mv_sign`, `mv_class`, `mv_class0_bit`, `mv_bit`, `mv_class0_fr`, `mv_class0_hp`, `mv_fr`, `mv_hp`). Two §9.3 step-2 subtleties are implemented and pinned by anchored tests: the **corner-inferred `PARTITION_SPLIT`** (`hasRows == 0 && hasCols == 0` — the §9.3.1 tree selection returns an integer without reading bits, and the counting process still runs) and the **absent hp bit** (`UseHp == 0` fixes `mv_hp` / `mv_class0_hp` to 1 bit-free — still counted). Elements the syntax table skips (seg-feature overrides, inferred tx sizes, non-SWITCHABLE filters) are correctly NOT counted; `segment_id` / `seg_id_predicted` / `default_intra_mode` / `default_uv_mode` are NA per the table (round 409)
- **`uv_mode_probs` joins the persisted `FrameContext` bank** — §7.1.2 `save_probs( )` persists "each probability table" and §8.4.4 `adapt_noncoef_probs( )` adapts `uv_mode_probs[ INTRA_MODES ][ … ]`, so the inter-frame `uv_mode` table (read by §6.4.15 intra blocks inside inter frames, §9.3.2 `ctx = y_mode`) is bank state, not a static default. `FrameContext` / `Vp9CompressedHeaderInter` now carry it (the §6.3 compressed header has no forward update for it — the working copy is the loaded bank's), the §6.4.15 decode reads the threaded copy instead of the static §10.5 table (bit-identical for the all-parallel-mode corpus, where the bank never leaves the defaults), and §8.4.4 adapts it per y-mode row (round 409)
...
- **corpus-found: unclipped §8.5.2 prediction store on frame-edge inter leaves** — a §6.4.3 `hasRows` / `hasCols`-admitted inter block that overhangs the visible frame (partial superblocks: any dimensions not a multiple of 64) wrote its predicted samples past the MiCols*8 x MiRows*8 working planes and panicked out-of-bounds on a **legal** stream; §8.5.1 `predict_intra` and §8.6.2 `reconstruct` already clipped their stores, §8.5.2 `predict_inter` now does the same. Found by (and pinned with) the new 176x144 `qcif-inter-gop` corpus fixture — 12 frames of continuous zoom, byte-exact after the fix (round 406)
- **§8.1 step-2 loop-filter gate: `loop_filter_level == 0` disables the whole §8.8 process** — the frame loop filter was invoked unconditionally after reconstruction; §8.1 step 2 runs it only "if loop_filter_level is not equal to 0". With `loop_filter_delta_enabled = 1`, the §8.8.1 `LvlLookup` init adds the ref/mode deltas to the (zero) frame level, so the default +1 INTRA ref-delta lifted intra edges to `lvl = 1` and filtered them even on level-0 frames — the `superframe-2` corpus divergence (scattered ±1 samples on every frame, all of which code `level=0` with live deltas). With the gate, `superframe-2` decodes **byte-exact across all 16 shown frames** (hidden alt-refs included) and every fixture in the 16-stream staged corpus is now byte-exact; pinned by `superframe2_keyframe_byte_exact` + the new full-sequence `superframe2_sequence_byte_exact` (round 406)
...
- **whole-corpus sequence validation** — new `full_corpus_sequences_byte_exact` test: all 16 staged fixtures decode fully byte-exact through `decode_vp9_sequence` (IVF demux → Annex B split → sequence decode). This corpus-validates inter paths previously covered only by unit tests: the profile-1/2/3 P-frames exercise §8.5.2 motion compensation at 4:4:4 chroma (no sub-8x8 MV averaging) and 10/12-bit depth (bit-depth-scaled convolution clamps), and the two `auto-alt-ref` streams carry 64 + 117 real compound (`ref_frame[1] > NONE`) blocks, so the §8.5.2 `Round2( p0 + p1, 1 )` compound average is now corpus-validated end-to-end (round 406)

- **corpus extension: 4:2:2 inter + intra-blocks-in-inter** — two new staged fixtures generated with the black-box reference binaries (commands + SHA-256 in each fixture's notes.md): `profile-1-yuv422-8bit-inter` (the corpus's first 4:2:2 stream — keyframe + 3 P-frames driving the `subsampling_x=1 / subsampling_y=0` chroma geometry through §8.5.2 motion compensation, §6.4.22 UV tx sizing and the §8.8 loop filter) and `intra-blocks-in-inter` (a mid-GOP scene cut under fixed keyframe interval, coding 62 intra blocks inside P-frames — the §6.4.13 `is_inter=0` arm, §6.4.15 intra mode info and §8.5.1 intra prediction inside an inter frame). Both decode byte-exact and join the whole-corpus sweep (round 406)
