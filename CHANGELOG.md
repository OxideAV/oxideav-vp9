...
- `show-existing-frame` corpus keyframe pinned byte-exact + documented inter divergence boundary — a docs-gated regression test demuxes the 24-frame `auto-alt-ref=2` `show-existing-frame` fixture (IVF → Annex B split) and confirms its keyframe decodes byte-exact against `expected.yuv` frame 0, then asserts that decoding through the first inter frame currently errors at the §9.2.3 `exit_bool` padding check (the `auto-alt-ref` inter tile decode consumes the wrong number of bits, so the hidden-ARF inter path is not yet bit-exact). The pin locks the correct keyframe boundary and is the precise reproduction for the follow-up inter-decode work; it flips to a full-sequence byte-exact assertion once the inter path lands (round 354)
